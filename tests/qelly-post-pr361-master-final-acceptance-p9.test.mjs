...
test('master acceptance: Decision is the live evidence workspace with complete R:R/lifecycle and explicit unavailable-evidence boundaries',async()=>{
  ...
  for(const phrase of ['Find Trade Now','1:1','1:2','1:3','1:4','Auto','Custom','Past','Present','Future','Ask QELLY','NO TRADE'])assert.ok(ui.toLowerCase().includes(phrase.toLowerCase()),`missing Decision UI phrase: ${phrase}`);
