...
const apiRows=apiRoutes.map((route,index)=>({index:index+1,route,access:route.startsWith('/api/v1/public/')||['/api/health','/api/ready','/api/v1/config'].includes(route)?'public':'authenticated-or-policy-dependent',implementation:'documented-contract'}));
