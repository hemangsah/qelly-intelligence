...
export async function renderLiveMarkets(main,deps){
  ...
  <section class="q-panel q-market-tape"><div class="q-panel-head"><div><h2>Live watch universe</h2><p>Provider-ready symbols with animated price surfaces</p></div><button class="q-button q-button--secondary" data-action="open-watchlists">Open watchlists</button></div><div class="q-panel-body"><div class="q-tape-grid">${['BTCUSDT','ETHUSDT','BNBUSDT','SOLUSDT','XRPUSDT','ADAUSDT'].map((symbol,index)=>`<button class="q-tape-card" data-live-symbol="${symbol}"><span>${symbol.replace('USDT',' / USDT')}</span><strong>${['₹65.1L','₹3.42L','₹63,100','₹17,450','₹268','₹92'][index]}</strong><small class="${index%3===0?'is-negative':'is-positive'}">${index%3===0?'−0.42%':`+${(index*.37+0.58).toFixed(2)}%`}</small></button>`).join('')}</div></div></section>
  </section>`;
