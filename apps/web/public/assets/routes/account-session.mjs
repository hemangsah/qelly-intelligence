...
export async function renderAccountSession(main,{api,pageHead,escapeHtml,toast,onLoggedOut,onAuthenticated}){
  ...
  main.innerHTML=`<section class="q-page q-v6-account-page">
    ${pageHead('Qelly Intelligence · Identity','Profile & Security','Manage the authenticated Qelly profile, research preferences and current browser session. Unsupported identity controls remain explicitly unavailable until their production capability is proven.',`<button class="q-button q-button--secondary" data-refresh>Refresh session</button><button class="q-button q-button--primary" data-save-profile>Save profile</button>`)}
