...
OUT=ROOT/'preview'/'release-a3-screenshots'
...
const environment={...process.env,
 ...
 QELLY_SESSION_SECRET:'release-a3-browser-session-secret-000000000001',QELLY_PASSWORD_PEPPER:'release-a3-browser-pepper'};
...
try:
    ...
    cases=[
      ...
      ('passkey-center-desktop','passkey-center',{'width':1440,'height':1000},True),
      ('passkey-center-mobile','passkey-center',{'width':390,'height':844},True),
      ('account-recovery-desktop','account-recovery',{'width':1440,'height':1000},True),
      ('account-recovery-mobile','account-recovery',{'width':390,'height':844},True),
      ('delivery-operations-desktop','delivery-operations',{'width':1440,'height':1000},True),
      ('delivery-operations-mobile','delivery-operations',{'width':390,'height':844},True),
    ]
    results=[]
    with sync_playwright() as p:
        ...
        for index,(name,route,viewport,authenticated) in enumerate(cases):
            ...
            if authenticated:
                ...
                payload=json.dumps({'email':unique,'password':'Qelly-Browser-Foundation-2026!','displayName':'Browser Validator','organizationName':f'Browser Validation Org {index} {int(time.time()*1000)}','workspaceName':'Release A3 Workspace','locale':'en-US','timezone':'UTC','baseCurrency':'USD'}).encode()
    ...
    sheet_path=ROOT/'preview'/'QELLY_RELEASE_A3_CONTACT_SHEET.jpg'; sheet.save(sheet_path,quality=92)
    failures=[x for x in results if x['consoleErrors'] or not x['heading'] or x['horizontalOverflowPx']>2 or not x['hash'].startswith('#/')]
    log={'release':'25.0.0','generatedAt':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'browser':'Chromium via Python Playwright','renderCount':len(results),'consoleErrorCount':sum(len(x['consoleErrors']) for x in results),'contactSheet':str(sheet_path.relative_to(ROOT)),'renders':results,'status':'passed' if not failures else 'failed'}
    (ROOT/'preview'/'RELEASE_A3_BROWSER_RENDER_LOG.json').write_text(json.dumps(log,indent=2)+'\n')
