...
const MODES=Object.freeze([
  ...
  {id:'decision',step:'04',title:'Decide',copy:'Stress a thesis, expose contradictions and hand the result into Decision Intelligence for human judgment.',prompt:'Help me frame a falsifiable Bitcoin decision thesis. Include supporting evidence, contradictions, uncertainty and an invalidation condition.'}
]);
...
export async function renderQellyChatWorkspace(main,{api,pageHead,stateBanner,escapeHtml,navigate}){
  ...
  main.innerHTML=`<section class="q-page q-chat-workspace">${pageHead('Qelly flagship intelligence workspace','Qelly Chat','Research, compare, explain and decide with connected evidence. Every available current claim exposes its source state; every missing source stays visible.',`<button class="q-button q-button--secondary" type="button" data-chat-action="evidence">Inspect evidence</button><button class="q-button q-button--primary" type="button" data-chat-action="open">Open full Qelly Chat</button>`)}${stateBanner()}
    ...
    <section class="q-chat-handoff"><div><span>Evidence</span><strong>Inspect source state</strong><button type="button" data-route="qelly-verify">Open Verify</button></div><div><span>Decision</span><strong>Review the research view</strong><button type="button" data-route="decision-provenance">Open Decision Intelligence</button></div><div><span>Research</span><strong>Organize the dossier</strong><button type="button" data-route="research-workspace">Open Research Workspace</button></div></section>
