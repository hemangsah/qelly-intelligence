...
const advancedQuantMarkup=(data,escapeHtml)=>{
  const metrics=data.advancedMetrics||{};
  const cards=[
    ['Garman–Klass vol',metrics.garmanKlassVolatilityPct==null?'Unavailable':pct(metrics.garmanKlassVolatilityPct)],
    ['Rogers–Satchell vol',metrics.rogersSatchellVolatilityPct==null?'Unavailable':pct(metrics.rogersSatchellVolatilityPct)],
    ['Downside deviation',metrics.downsideDeviationPct==null?'Unavailable':pct(metrics.downsideDeviationPct)],
    ['Trend efficiency',metrics.efficiencyRatio==null?'Unavailable':Math.round(metrics.efficiencyRatio*100)+'%'],
    ['ROC 14',metrics.roc14Pct==null?'Unavailable':pct(metrics.roc14Pct)],
    ['Bollinger z 20',metrics.bollingerZ20==null?'Unavailable':String(metrics.bollingerZ20)],
    ['Range position',metrics.rangePosition20Pct==null?'Unavailable':pct(metrics.rangePosition20Pct)],
    ['Volatility regime',String(metrics.volatilityRegime||'unavailable')]
  ];
  return '<section class="q-dpg-quant"><header><div><small>ADVANCED QUANT STATE</small><h2>Distribution, range and regime</h2></div><span>Derived from validated observed candles</span></header><div>'+cards.map(([label,value])=>'<article><span>'+escapeHtml(label)+'</span><strong>'+escapeHtml(value)+'</strong></article>').join('')+'</div></section>';
};

const historicalAnalogsMarkup=(data,escapeHtml)=>{
  const analogs=data.historicalAnalogs;
  if(!analogs||analogs.state!=='derived'||!analogs.summary)return '<section class="q-dpg-analogs"><header><div><small>HISTORICAL ANALOGS</small><h2>Not enough comparable history</h2></div></header><p>Historical similarity remains unavailable rather than being inferred.</p></section>';
  const s=analogs.summary;
  return '<section class="q-dpg-analogs"><header><div><small>HISTORICAL ANALOGS</small><h2>'+escapeHtml(String(analogs.count))+' nearest prior states</h2></div><span>'+escapeHtml(String(analogs.lookbackBars))+'-bar context → '+escapeHtml(String(analogs.horizonBars))+'-bar observed outcome</span></header><div class="q-dpg-analog-summary"><article><span>Median outcome</span><strong>'+pct(s.medianOutcomePct)+'</strong></article><article><span>P25 / P75</span><strong>'+pct(s.p25OutcomePct)+' / '+pct(s.p75OutcomePct)+'</strong></article><article><span>Positive analogs</span><strong>'+Math.round(s.positiveRate*100)+'%</strong></article><article><span>Negative analogs</span><strong>'+Math.round(s.negativeRate*100)+'%</strong></article></div><details><summary>Inspect analog sample</summary><ol>'+analogs.analogs.map(item=>'<li><span>'+new Date(item.startTime).toLocaleDateString()+' → '+new Date(item.endTime).toLocaleDateString()+'</span><strong>'+pct(item.outcomePct)+'</strong><small>distance '+escapeHtml(String(item.distance))+'</small></li>').join('')+'</ol><p>'+escapeHtml(analogs.methodology)+'</p></details><p class="q-dpg-confidence-note">Historical analog outcomes are descriptive comparisons, not calibrated probabilities or guarantees.</p></section>';
};
...
export async function renderDecisionProvenGraph(main,deps){
  ...
  const hero=(data)=>{
    ...
    return '<section class="q-dpg-hero q-dpg-hero--'+actionTone(action)+'" aria-label="QELLY Decision Intelligence">'+
      ...
      '</div></div>'+
      '<div class="q-dpg-hero__actions"><button class="q-button q-button--primary" data-dpg-find-trade>Find Trade Now</button><button class="q-button q-button--secondary" data-dpg-explain-header '+(state.draft?'':'disabled')+'>Explain This Move</button><button class="q-button q-button--secondary" data-dpg-mtf-jump>Compare Timeframes</button><a class="q-button q-button--secondary" href="#/qelly-chat">Open QELLY Chat</a><button class="q-button q-button--secondary" data-dpg-methodology-jump>Methodology / Sources</button></div>'+
    '</section>';
  };
  const evidence=(data)=>{
  ...
  const content=(data)=>{
    ...
    return '<section class="q-dpg-truth"><span class="q-status q-status--'+(data.truthState==='LIVE'?'live':data.truthState.toLowerCase())+'">'+escapeHtml(data.truthState)+'</span><strong>'+escapeHtml(data.asset)+' / '+escapeHtml(data.interval)+'</strong><span>'+escapeHtml(data.market.currentState.label)+' · updated '+new Date(data.observedAt).toLocaleString()+'</span></section>'+
      ...
      multiTimeframe(data,escapeHtml)+advancedQuantMarkup(data,escapeHtml)+historicalAnalogsMarkup(data,escapeHtml)+derivativesContext(data,escapeHtml)+adSlot('decision-intelligence-inline')+'<section class="q-dpg-evidence"><header><div><small>EVIDENCE RANKING</small><h2>What best explains the move</h2></div><span>News: '+escapeHtml(data.evidence?.news?.state||'unavailable')+' · Funding/OI: '+escapeHtml(data.evidence?.derivatives?.state||'unavailable')+' · Liquidations: unavailable, not inferred</span></header>'+evidence(data)+'</section>'+
