...
export async function renderIntelligenceTerminal(main,deps){
  ...
  const [providersResult,networkResult,healthResult,chatResult]=await Promise.allSettled([
    api('/api/v1/providers/status'),
    api('/api/v1/market/network'),
    api('/api/v1/health'),
    api('/api/v1/intelligence/chat')
  ]);
  ...
  const chatCapability=chatResult.status==='fulfilled'?chatResult.value:{};
  ...
  main.innerHTML=`<section class="q-page q-intelligence-terminal" data-intelligence-terminal="v1">
    ${pageHead('Qelly Intelligence · AI, news and community','Intelligence Terminal','Evidence-grounded Qelly AI, governed finance datasets, official newsrooms, external provider launches and a private research stream.',`<button class="q-button q-button--primary" type="button" data-open-qelly-ai>Ask Qelly AI</button><a class="q-button q-button--secondary" href="#/market">Open Market Command</a>`)}${stateBanner()}
    <section class="q-it-hero"><div class="q-it-hero__grid" aria-hidden="true"></div><div class="q-it-hero__copy"><span class="q-it-kicker"><i></i> INTELLIGENCE ${chatCapability?.assistant?.available?'ONLINE':'DEGRADED'}</span><h2>Ask the market.<br><em>Inspect the evidence.</em></h2><p>Qelly Intelligence combines model reasoning with connected crypto, global macro and ECB reference observations. Every response keeps source, freshness, access and execution boundaries visible.</p></div><div class="q-it-radar" aria-hidden="true"><span>Q</span><i style="--i:0">AI</i><i style="--i:1">NEWS</i><i style="--i:2">DATA</i><i style="--i:3">EVIDENCE</i></div></section>
    <section class="q-it-guide" aria-labelledby="q-it-guide-title"><div><p class="q-eyebrow">Qelly Intelligence · grounded finance AI</p><h2 id="q-it-guide-title">What do you want to investigate?</h2><p>Questions are processed by Qelly’s Cloudflare AI service and grounded against governed dataset observations whenever current data is required.</p></div><form data-it-guide><label><span class="q-visually-hidden">Research request</span><textarea name="prompt" rows="2" maxlength="2400" placeholder="Try: compare India, US and China GDP growth and inflation…"></textarea></label><button class="q-button q-button--primary" type="submit">Ask Qelly</button></form><div class="q-it-guide__answer" data-it-answer aria-live="polite"><strong>Ready for a finance research question.</strong><span>${chatCapability?.datasets?.connected??4} datasets connected · ${chatCapability?.datasets?.catalogued??24} governed entries · no trade execution.</span></div></section>
    <div class="q-it-tabs" role="tablist" aria-label="Intelligence terminal sections"><button type="button" role="tab" aria-selected="true" aria-controls="q-it-ai" id="q-it-tab-ai" data-it-tab="ai">Qelly AI + Network</button><button type="button" role="tab" aria-selected="false" aria-controls="q-it-news" id="q-it-tab-news" data-it-tab="news">Newsroom</button><button type="button" role="tab" aria-selected="false" aria-controls="q-it-community" id="q-it-tab-community" data-it-tab="community">Community Desk</button><button type="button" role="tab" aria-selected="false" aria-controls="q-it-evidence" id="q-it-tab-evidence" data-it-tab="evidence">Provider Evidence</button></div>
    <section id="q-it-ai" class="q-it-panel" role="tabpanel" aria-labelledby="q-it-tab-ai" data-it-panel="ai"><header><div><p class="q-eyebrow">Qelly AI and provider network</p><h2>Use Qelly’s grounded assistant or open a separate specialist</h2></div><span class="q-status q-status--${chatCapability?.assistant?.inferenceAvailable?'live':'cached'}">${chatCapability?.assistant?.inferenceAvailable?'GROUNDED AI ONLINE':'DATASET MODE'}</span></header><div class="q-it-provider-grid">${providerCards(providers,chatCapability)}</div></section>
  ...
  form?.addEventListener('submit',(event)=>{event.preventDefault();const prompt=String(new FormData(form).get('prompt')||'').trim();if(!prompt){toast?.('Enter a finance research question.',{tone:'danger'});return;}document.dispatchEvent(new CustomEvent('qelly:open-ai',{detail:{prompt}}));const answer=main.querySelector('[data-it-answer]');answer.innerHTML='<strong>Qelly Intelligence opened.</strong><span>Your question is ready in the evidence-grounded chat box.</span>';});
  main.querySelectorAll('[data-open-qelly-ai]').forEach((button)=>button.addEventListener('click',()=>document.dispatchEvent(new CustomEvent('qelly:open-ai'))));
