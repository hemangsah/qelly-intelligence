...
try:
        ...
        for vname, viewport in viewports:
            ...
            for route_key, route_path, auth in routes:
                ...
                def proxy(route_obj,is_auth=auth,current_route=route_key):
                    ...
                    if parsed.netloc=='qelly.test' and parsed.path=='/' and route_obj.request.resource_type=='document': route_obj.fulfill(status=200,headers={'Content-Type':'text/html; charset=utf-8'},body=INDEX); return
                    if parsed.netloc=='qelly.test' and parsed.path=='/assets/fonts/ibm-plex-sans-variable.woff2': route_obj.fulfill(status=200,headers={'Content-Type':'font/woff2','Cache-Control':'no-store'},path=str(COMPILED_FONT)); return
                    ...
                    if not is_auth and parsed.path=='/api/v1/auth/status': route_obj.fulfill(status=200,headers={'Content-Type':'application/json; charset=utf-8'},body=json.dumps({'authenticated':False,'mode':'production-cookie','productionFoundation':{'developmentIdentityEnabled':False}})); return
                ...
                try:
                    page.goto(f'https://qelly.test/#/{route_path}',wait_until='load',timeout=30000); page.wait_for_selector('main#main h1',timeout=15000); page.wait_for_timeout(180); checks=page.evaluate(EVALUATE); page.keyboard.press('Tab'); focus=page.evaluate("({tag:document.activeElement?.tagName,id:document.activeElement?.id||null})")
                ...
                results.append({'route':route_key,'path':route_path,'viewport':vname,'dimensions':viewport,'authenticated':auth,'checks':checks,'firstTabFocus':focus,'consoleErrors':errors,'criticalFailures':failures,'status':'passed' if not failures else 'failed'}); context.close()
        browser.close()
    ...
    log={'release':'Prompt 2B final','generatedAt':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'method':'automated semantic, keyboard-entry, exact-font and responsive regression; not an independent WCAG certification','routeCount':len(routes),'viewportCount':len(viewports),'expectedChecks':expected_checks,'checks':len(results),'passed':len(results)-len([item for item in results if item['status']=='failed']),'failed':len(failed),'results':results,'status':'passed' if not failed else 'failed'}
