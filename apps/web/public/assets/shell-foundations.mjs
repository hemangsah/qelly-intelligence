...
export function renderShellFoundations({
  ...
  if(shelf){
    shelf.innerHTML=`<nav class="q-breadcrumbs" aria-label="Breadcrumb"><button type="button" data-domain-home="${resolvedDomain?.id??'markets'}">Qelly</button><span aria-hidden="true">/</span><strong>${escapeHtml(resolvedDomain?.label??'Markets')}</strong><span aria-hidden="true">/</span><span aria-current="page">${escapeHtml(current?.label??'Overview')}</span></nav><div class="q-category-shelf" aria-label="${escapeHtml(resolvedDomain?.label??'Product')} destinations">${siblings.slice(0,6).map((route)=>`<button type="button" data-shelf-route="${route.route}" class="${route.route===currentRoute?'is-active':''}" aria-current="${route.route===currentRoute?'page':'false'}">${escapeHtml(route.label)}</button>`).join('')}</div><div id="shell-context-contract" class="q-terminal-context" aria-label="Typed route and evidence context" aria-live="polite"><span class="q-terminal-context__token"><small>Context</small><strong>${escapeHtml(current?.kind??'workspace')}</strong></span><span class="q-terminal-context__token"><small>Time</small><strong>${escapeHtml(persona.defaultTimeframe)}</strong></span><span class="q-terminal-context__token"><small>Evidence</small><strong>${staticVisualPreview?'Simulated':'Panel-owned'}</strong></span></div>`;
  ...
  if(compare){
    compare.setAttribute('aria-label','Activity tray and persistent intelligence actions');
    compare.innerHTML=`<div id="shell-activity-contract" class="q-terminal-activity" aria-label="Operational activity"><span class="q-terminal-activity__item is-safe"><small>Safety</small><strong>READ ONLY</strong></span><span class="q-terminal-activity__item"><small>Session</small><strong>${staticVisualPreview?'Anonymous':'Resolving'}</strong></span><span class="q-terminal-activity__item"><small>Evidence</small><strong>${staticVisualPreview?'Simulated':'Panel-owned'}</strong></span></div><div class="q-terminal-activity-actions"><button type="button" data-watchlist-open>Watchlist</button><button type="button" data-compare-open>Compare</button><button type="button" data-explain-open>Explain</button></div>`;
