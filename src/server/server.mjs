...
async function requireCsrf(runtime, request, sid, pathname) {
  ...
  if (['/api/v1/auth/register','/api/v1/auth/login','/api/v1/auth/passkeys/authenticate/options','/api/v1/auth/passkeys/authenticate/verify','/api/v1/auth/recovery/request','/api/v1/auth/recovery/reset','/api/v1/calculations/run','/api/v1/calculations/batch','/api/v1/indicators/run','/api/v1/india/charges'].includes(pathname)) return;
...
export function createServer({runtime=createRuntime(defaultRuntimeDir)}={}){
  return http.createServer(async (request,response)=>{
    ...
    try{
      ...
      if(request.method==='GET' && url.pathname==='/api/v1/calculations/metadata') return json(response,200,runtime.calculationService.metadata(),id);
      if(request.method==='GET' && url.pathname==='/api/v1/calculations/formulas') return json(response,200,runtime.calculationService.formulas({domain:url.searchParams.get('domain')}),id);
      if(request.method==='GET' && /^\/api\/v1\/calculations\/formulas\/[^/]+$/.test(url.pathname)) return json(response,200,runtime.calculationService.formula(decodeURIComponent(url.pathname.split('/').at(-1))),id);
      if(request.method==='POST' && url.pathname==='/api/v1/calculations/run') { const body=await bodyJson(request,1_000_000); runtime.schemaRegistry.validate('calculation-request',body,{status:400,code:'request_schema_invalid'}); return json(response,200,runtime.calculationService.calculate(body),id); }
      if(request.method==='POST' && url.pathname==='/api/v1/calculations/batch') return json(response,200,runtime.calculationService.batch(await bodyJson(request,1_000_000)),id);
      if(request.method==='GET' && url.pathname==='/api/v1/indicators') return json(response,200,runtime.calculationService.indicators({category:url.searchParams.get('category')}),id);
      if(request.method==='GET' && /^\/api\/v1\/indicators\/[^/]+$/.test(url.pathname)) return json(response,200,runtime.calculationService.indicator(decodeURIComponent(url.pathname.split('/').at(-1))),id);
      if(request.method==='POST' && url.pathname==='/api/v1/indicators/run') { const body=await bodyJson(request,1_000_000); runtime.schemaRegistry.validate('indicator-request',body,{status:400,code:'request_schema_invalid'}); return json(response,200,runtime.calculationService.calculateIndicator(body),id); }
