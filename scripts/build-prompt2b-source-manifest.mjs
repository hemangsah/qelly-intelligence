...
const classify=file=>file.startsWith('.github/workflows/')?'workflow':file.startsWith('apps/web/public/assets/routes/')?'route':file.startsWith('apps/web/public/assets/calculation/')?'numerical-source':file.startsWith('packages/schemas/')?'schema':file.startsWith('packages/migrations/')?'migration':file.startsWith('tests/')?'test':file.startsWith('scripts/')?'governance-script':file.startsWith('project-state/')?'durable-state':file.startsWith('figma-plugin/')||file.startsWith('design/')||file.startsWith('QELLY_')?'design-inventory':file.startsWith('src/')?'server-source':'repository-file';
const files=[];const aggregate=createHash('sha256');
for(const file of paths){
  const full=path.join(root,file),body=await readFile(full),info=await stat(full),gitBlobSha=git(['hash-object',file]).trim(),fetched=git(['cat-file','blob',gitBlobSha],{encoding:'buffer'}),introducing=git(['log','--follow','--diff-filter=A','--format=%H','--',file]).trim().split(/\r?\n/).filter(Boolean).at(-1)??'UNAVAILABLE',latest=git(['log','-1','--format=%H','--',file]).trim()||'UNAVAILABLE',hash=createHash('sha256').update(body).digest('hex'),fetchedHash=createHash('sha256').update(fetched).digest('hex');
