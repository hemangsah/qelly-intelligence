...
export async function renderLiveMarkets(main,deps){
  ...
  const provider=authorized[0]||null;
  const providerOptions=authorized.map((item)=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join('');

  main.innerHTML=`<section class="q-page q-live-market-page" data-market-runtime="pages-contract-v3-no-fabrication">
    ${pageHead('Qelly Intelligence · Governed market display','Live Market Command','External market display and Qelly-governed analytical observations are separated. When internal provider rights are unavailable, Qelly shows unavailable rather than generated prices or candles.',`<a class="q-button q-button--ghost" href="#/data-mesh">Provider policy</a><button class="q-button q-button--primary" data-action="refresh-live">Refresh status</button>`)}${stateBanner()}

    <section class="q-live-command-deck" data-v53-evidence-adjacent="true">
      <div class="q-live-symbol-block"><span class="q-live-asset-icon">Q</span><div><p>External display market</p><h2 id="live-symbol-title">${escapeHtml(symbolLabel(DISPLAY_SYMBOLS[0]))}</h2><span>TradingView · display-only boundary</span></div></div>
      <div class="q-live-price-block"><span id="live-last-price">—</span><strong id="live-change">INTERNAL FEED ${provider?'AVAILABLE':'UNAVAILABLE'}</strong><small id="live-range">No Qelly-generated fallback values</small></div>
      <div class="q-live-controls">
        <label><span>Display symbol</span><select id="live-symbol">${DISPLAY_SYMBOLS.map(symbol=>`<option value="${symbol}">${escapeHtml(symbolLabel(symbol))}</option>`).join('')}</select></label>
        <label><span>Interval</span><select id="live-interval">${INTERVALS.map(interval=>`<option value="${interval}">${interval}</option>`).join('')}</select></label>
        <label><span>Governed provider</span><select id="live-provider" ${provider?'':'disabled'}>${providerOptions||'<option>No rights-authorized provider</option>'}</select></label>
        <button class="q-live-toggle" id="live-stream-toggle" type="button" disabled aria-disabled="true"><span></span><b>${provider?'Provider approved':'Streaming unavailable'}</b></button>
      </div>
    </section>
    ...
    <section class="q-v5-market-ribbon" aria-label="Market data boundaries">
      ...
      <div class="q-v5-market-metric"><span>Internal provider</span><strong id="live-kpi-provider">${provider?escapeHtml(provider.id.toUpperCase()):'UNAVAILABLE'}</strong></div>
    ...
    <section class="q-v5-evidence-ribbon" aria-label="Market evidence envelope">
      <div class="q-v5-evidence-cell"><span>Analytics source</span><strong id="live-evidence-source">${provider?escapeHtml(provider.name):'No authorized crypto feed'}</strong></div>
      <div class="q-v5-evidence-cell"><span>Observed</span><strong id="live-evidence-observed">Not supplied</strong></div>
      <div class="q-v5-evidence-cell"><span>Truth state</span><strong id="live-evidence-truth">${provider?'Awaiting provider':'UNAVAILABLE'}</strong></div>
      <div class="q-v5-evidence-cell"><span>Display source</span><strong>TradingView · external</strong></div>
    ...
    <div class="q-live-layout">
      ...
      <aside class="q-live-side-stack">
        <section class="q-panel"><div class="q-panel-head"><div><h2>Governed market status</h2><p>Internal analytical data only</p></div><span class="q-status q-status--${provider?'live':'unavailable'}">${provider?'AUTHORIZED':'UNAVAILABLE'}</span></div><div class="q-panel-body"><div class="q-live-stat"><span>Last</span><strong id="live-stat-last">—</strong></div><div class="q-live-stat"><span>Change</span><strong id="live-stat-change">—</strong></div><div class="q-live-stat"><span>High / low</span><strong id="live-stat-range">—</strong></div><div class="q-live-stat"><span>Volume</span><strong id="live-stat-volume">—</strong></div><div class="q-live-stat"><span>Provider rights</span><strong>${provider?'Authorized':'Blocked / unverified'}</strong></div><div class="q-live-stat"><span>Execution</span><strong class="is-negative">Disabled</strong></div></div></section>
  ...
  const mountExternal=()=>{chartHandle?.destroy?.();chartHandle=mountTradingViewDisplay(chartTarget,{symbol:controls.symbol.value,interval:controls.interval.value});main.querySelector('#live-symbol-title').textContent=symbolLabel(controls.symbol.value);};
  ...
  const loadGoverned=async()=>{
    if(!provider){clearGoverned();return;}
    const providerId=controls.provider.value||provider.id;
    const symbol=controls.symbol.value;
    try{
      const data=await api(`/api/v1/live-markets/candles?provider=${encodeURIComponent(providerId)}&symbol=${encodeURIComponent(symbol)}&interval=${encodeURIComponent(controls.interval.value)}&limit=120`);
      const summary=data.summary||{},source=data.source||{};
      main.querySelector('#live-last-price').textContent=format(summary.last);main.querySelector('#live-kpi-last').textContent=format(summary.last);main.querySelector('#live-kpi-change').textContent=Number.isFinite(Number(summary.changePercent))?`${Number(summary.changePercent)>=0?'+':''}${Number(summary.changePercent).toFixed(2)}%`:'—';main.querySelector('#live-stat-last').textContent=format(summary.last);main.querySelector('#live-stat-change').textContent=format(summary.change);main.querySelector('#live-stat-range').textContent=summary.high==null?'—':`${format(summary.high)} / ${format(summary.low)}`;main.querySelector('#live-stat-volume').textContent=format(summary.volume);main.querySelector('#live-evidence-source').textContent=source.name||providerId;main.querySelector('#live-evidence-observed').textContent=source.observedAt?new Date(source.observedAt).toLocaleString('en-IN'):'Not supplied';main.querySelector('#live-evidence-truth').textContent=String(source.mode||'UNAVAILABLE').toUpperCase();
    }catch(error){clearGoverned();toast?.(`Governed provider data unavailable: ${error.message}`,{tone:'warning'});}
  };
  const refresh=async()=>{mountExternal();await loadGoverned();};
  controls.symbol?.addEventListener('change',refresh);controls.interval?.addEventListener('change',refresh);controls.provider?.addEventListener('change',loadGoverned);main.querySelector('[data-action="refresh-live"]')?.addEventListener('click',loadGoverned);
