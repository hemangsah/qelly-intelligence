...
const featureItems=routeDefinitions.filter((item)=>!item.hidden&&!item.anonymousOnly).map((item,index)=>({
  ...
  keywords:`${item.section} ${item.domain} ${item.kind} ${item.meta} ${item.route==='decision-provenance'?'decision intelligence current supported research evidence methodology sources contradictions invalidation risk reward calibration historical analogs what changed buy sell wait no trade':''}`,
  route:item.route,
