...
const probabilityCalibrationMarkup=(data,escapeHtml)=>{
  ...
  return '<section class="q-dpg-model-calibration"><header><div><small>MODEL CALIBRATION · WALK-FORWARD</small><h2>'+escapeHtml(String(calibration.state||'UNCALIBRATED').replaceAll('_',' '))+'</h2><p>'+escapeHtml(calibration.reason||'Calibration evidence is unavailable.')+'</p></div><span>'+(calibration.eligible?'ELIGIBLE':diagnosticOnly?'DIAGNOSTIC ONLY':'NOT ELIGIBLE')+'</span></header>'+
    '<div class="q-dpg-model-calibration__metrics"><article><span>Resolved samples</span><strong>'+escapeHtml(String(calibration.sampleSize??0))+'</strong><small>independent sample gate '+escapeHtml(sampleGate)+'</small></article><article><span>Outcome separation</span><strong>'+escapeHtml(separation)+'</strong><small>overlap '+(calibration.outcomeWindowOverlap===true?'YES':'NO')+'</small></article><article><span>Brier score</span><strong>'+escapeHtml(brier)+'</strong><small>'+(diagnosticOnly?'diagnostic only':'lower is better')+'</small></article><article><span>Skill vs uniform</span><strong>'+escapeHtml(skill)+'</strong><small>'+(diagnosticOnly?'diagnostic only':'calibration diagnostic')+'</small></article><article><span>Reliability gap</span><strong>'+escapeHtml(gap)+'</strong><small>'+(diagnosticOnly?'diagnostic only':'lower is better')+'</small></article></div>'+
