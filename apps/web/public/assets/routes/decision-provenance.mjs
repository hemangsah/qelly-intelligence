...
function graphMarkup(graph,isDemo,escapeHtml,selectedNodeId){
  ...
  const nodeButtons=nodes.map((node)=>`<button type="button" class="q-provenance-node${node.nodeId===selected.nodeId?' is-selected':''}" data-provenance-node="${escapeHtml(node.nodeId)}" aria-selected="${node.nodeId===selected.nodeId?'true':'false'}" aria-controls="q-provenance-focus"><span class="q-provenance-node__meta"><span class="q-status q-status--${tone(node.type)}">${escapeHtml(node.type)}</span><small>r${escapeHtml(String(node.revision))}</small></span><strong>${escapeHtml(node.label)}</strong><small>${escapeHtml(node.classification)}</small></button>`).join('');
  ...
  return `<section class="q-panel q-provenance-explorer" data-provenance-explorer><div class="q-panel-head"><div><p class="q-eyebrow">Evidence graph · traversal mode</p><h2>${escapeHtml(graph.title)}</h2><p>${escapeHtml(graph.truthBoundary)}</p></div><span class="q-status q-status--${isDemo?'simulated':graph.integrity?.valid?'live':'unavailable'}">${isDemo?'demo structure':graph.integrity?.valid?'integrity valid':'integrity unavailable'}</span></div><div class="q-panel-body q-provenance-panel-body">
    <div class="q-provenance-workbench">
      <nav class="q-provenance-node-navigator" aria-label="Evidence graph records"><div class="q-provenance-column-head"><span>Records</span><strong>${nodes.length}</strong></div><div class="q-provenance-node-list">${nodeButtons}</div><p class="q-provenance-keyboard-hint">Use Tab to enter the graph. Arrow keys move between records.</p></nav>
      <article class="q-provenance-focus" id="q-provenance-focus" data-provenance-focus aria-live="polite" aria-atomic="true"><div class="q-provenance-focus__head"><div><p class="q-eyebrow">Selected record</p><h3>${escapeHtml(selected.label)}</h3></div><span class="q-status q-status--${tone(selected.type)}">${escapeHtml(selected.type)}</span></div><div class="q-provenance-record-meta"><span>${escapeHtml(selected.classification)}</span><span>Revision ${escapeHtml(String(selected.revision))}</span><span>${escapeHtml(selected.nodeId)}</span></div>${detailMarkup}</article>
    ...
    <div class="q-provenance-context-ribbon" data-provenance-context aria-live="polite"><span class="q-status q-status--${tone(selected.type)}">${escapeHtml(selected.type)}</span><strong>${escapeHtml(selected.label)}</strong><small>${connected.length} governed relationship${connected.length===1?'':'s'} · revision ${escapeHtml(String(selected.revision))}</small></div>
  </div></section>
