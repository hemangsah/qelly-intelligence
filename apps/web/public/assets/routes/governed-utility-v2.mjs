...
export async function renderGovernedTrustCenterV2(main,deps){
  ...
  const [capabilitiesResult,overviewResult]=await Promise.allSettled([api('/api/v1/platform/capabilities'),api('/api/v1/public/markets/overview')]);
  ...
  main.innerHTML=`<section class="q-page q-governed-trust-center" data-trust-runtime="governed-v2">
    ${pageHead('Qelly Intelligence · Trust Center','Trust Center','See coverage availability and the safeguards that keep Qelly research outputs transparent.',professionalLinks())}${stateBanner()}
    <section class="q-v7-boundary-ribbon"><div><span>Coverage in progress</span><strong>${unavailableCount}</strong><small>Clearly identified</small></div><div><span>Invented market values</span><strong>${overview?.guardrails?.fabricatedObservations===false?'OFF':'—'}</strong><small>No hidden substitutes</small></div><div><span>Workspace privacy</span><strong>PROTECTED</strong><small>Private data stays account scoped</small></div><div><span>Product mode</span><strong>RESEARCH</strong><small>No trade execution</small></div></section>
    <section class="q-panel"><div class="q-panel-head"><div><h2>Coverage availability</h2><p>Product areas appear as their verified data sources become available.</p></div></div><div class="q-panel-body q-stack">${unavailable.slice(0,8).map((item)=>`<div class="q-record-row"><span><strong>${escapeHtml(item.label||humanizeOperationalState(item.id).replace(/\.$/,''))}</strong><small>Coverage is being prepared.</small></span><span class="q-status q-status--cached">COMING SOON</span></div>`).join('')||'<p>All listed coverage is ready.</p>'}</div></section>
    <section class="q-panel"><div class="q-panel-head"><div><h2>Research safeguards</h2><p>How Qelly keeps displayed results understandable and auditable.</p></div><span class="q-status q-status--live">ACTIVE</span></div><div class="q-panel-body q-v7-evidence-strip"><span>Sources are named</span><span>Missing values stay empty</span><span>Reference timing is visible</span><span>Private workspaces stay private</span><span>${escapeHtml(truthLabel(overview.truthState||'UNAVAILABLE'))} market coverage</span></div></section>
  </section>`;
}

export const __governedUtilityV2Test=Object.freeze({formatNumber,formatTime});
import {humanizeOperationalState,truthLabel} from '../customer-copy.mjs';
