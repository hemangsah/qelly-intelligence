import json, pathlib, subprocess, tempfile, time, shutil, urllib.request, urllib.error, sys, os, signal
...
try:
    ...
    with sync_playwright() as p:
        ...
        for vname,viewport in viewports:
            ...
            for d in defs:
                ...
                (OUT/f'batch-{start:03d}-{end:03d}.json').write_text(json.dumps({'start':start,'end':end,'routeCount':len(defs),'renderCount':len(results),'results':results},indent=2)+'\n')
    ...
    failed=any(x['status']!='passed' for x in results)
finally:
    try: proc.terminate()
    except Exception: pass
    shutil.rmtree(runtime,ignore_errors=True)
# Chromium/Playwright can hang during graceful teardown in this constrained renderer.
# Kill only this setsid process group after durable evidence is written.
