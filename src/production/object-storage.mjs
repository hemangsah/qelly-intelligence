...
export class ClamAvTcpScanner{
  constructor({host='127.0.0.1',port=3310,timeoutMs=10000,connect=net.createConnection}={}){this.host=host;this.port=Number(port);this.timeoutMs=Number(timeoutMs);this.connect=connect;this.mode='clamav-tcp-instream';}
...
export class LocalObjectStorage{
  ...
  async health(){try{await mkdir(this.root,{recursive:true});return {ok:true,driver:this.mode,root:path.basename(this.root),scanner:this.scanner.mode,scannerStatus:this.scanner.status?.()??null,quarantine:'manual-and-automatic-local'};}catch(error){return {ok:false,driver:this.mode,error:error.message};}}
}
...
export class S3CompatibleObjectStorage{
  ...
  async request(method,key,{body=null,mimeType=null}={}){const url=this.objectUrl(key),payload=body??Buffer.alloc(0),payloadHash=sha256(payload),headers=this.signedHeaders({method,url,payloadHash,contentType:mimeType});const response=await this.fetch(url,{method,headers,body:['GET','DELETE','HEAD'].includes(method)?undefined:payload,signal:AbortSignal.timeout(10000)});if(!response.ok)throw Object.assign(new Error(`S3 request failed with HTTP ${response.status}`),{status:502,code:'object_storage_request_failed',details:{status:response.status}});return response;}
  ...
  async rescan(key,{fileName=null,mimeType='application/octet-stream'}={}){if(!String(key).startsWith('.quarantine/'))throw Object.assign(new Error('Only quarantined objects can be rescanned'),{status:409,code:'object_not_quarantined'});const staged=await this.get(key),scan=await this.scanner.scan(staged.content),parts=String(key).split('/'),tenantId=parts[1],workspaceId=parts[2],inferred=String(parts.at(-1)).split('-').slice(6).join('-')||'released-object.bin',name=safe(fileName??inferred),hash=sha256(staged.content),releasedKey=`${tenantId}/${workspaceId}/${hash}-${name}`;await this.request('PUT',releasedKey,{body:staged.content,mimeType});await this.request('DELETE',key);return {key:releasedKey,fileName:fileName??inferred,mimeType,size:staged.content.length,sha256:hash,scanner:scan.scanner,scanResult:scan.result??'clean',status:'released',quarantineStatus:'released',provider:this.mode,bucket:this.bucket};}
  ...
  async put(input){const item=normalizedInput(input,this.maxBytes),scan=await this.scanner.scan(item.buffer),key=`${safe(item.tenantId)}/${safe(item.workspaceId)}/${item.sha256}-${safe(item.fileName)}`;await this.request('PUT',key,{body:item.buffer,mimeType:item.mimeType});return {key,fileName:item.fileName,mimeType:item.mimeType,size:item.buffer.length,sha256:item.sha256,scanner:scan.scanner,scanResult:scan.result??'clean',status:'released',quarantineStatus:'scanned-before-upload',provider:this.mode,bucket:this.bucket};}
  ...
  async health(){try{const url=this.objectUrl('__qelly_health__');return {ok:true,driver:this.mode,endpoint:url.origin,bucket:this.bucket,region:this.region,connectivity:'configuration-validated',scanner:this.scanner.mode,scannerStatus:this.scanner.status?.()??null,quarantine:'manual-and-scan-before-release'};}catch(error){return {ok:false,driver:this.mode,error:error.message};}}
}
