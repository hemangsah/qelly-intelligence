...
export async function renderCalculatorDetail(main,{pageHead,escapeHtml,toast,navigate,id,query}){
  ...
  main.innerHTML=`<section class="q-page q-calculator-detail-page">
    ...
    <div class="q-state-banner" data-truth-state="deterministic"><span class="q-status" data-truth-state="deterministic">DETERMINISTIC</span><p>${sharedState?'Shared inputs restored. ':''}Calculations run locally using the documented Qelly formula. No provider, broker or exchange call is made.</p></div>
  ...
  const run=({announce=true,source='structured'}={})=>{
    ...
    form.querySelectorAll('[aria-invalid]').forEach((element)=>element.removeAttribute('aria-invalid'));renderOutputs(result);enableActions(true);document.dispatchEvent(new CustomEvent('qelly:product-event',{detail:{name:'calculator_complete',properties:{route:'calculator-detail',feature:definition.category??definition.domain??'calculator',action:source}}}));if(announce)toast(`${definition.name} calculated locally`,{tone:'success'});
  };
