...
export async function renderIntelligenceTerminal(main,deps){
  ...
  main.innerHTML=`<section class="q-page q-intelligence-terminal" data-intelligence-terminal="v1">
    ...
    <section id="q-it-evidence" class="q-it-panel" role="tabpanel" aria-labelledby="q-it-tab-evidence" data-it-panel="evidence" hidden><header><div><p class="q-eyebrow">Runtime evidence</p><h2>Every integration state, visible</h2></div><span class="q-status q-status--${stateTone(providersResult.status==='fulfilled')}">${providersResult.status==='fulfilled'?'CONNECTED':'DEGRADED'}</span></header><div class="q-it-evidence-grid">${providers.map((provider)=>`<article><div><strong>${esc(String(provider.id||'provider').toUpperCase())}</strong><span class="q-status q-status--${provider.enabled?'delayed':'unavailable'}">${provider.enabled?'REFERENCE':'BLOCKED'}</span></div><p>${esc(provider.termsState||provider.reason||'No policy detail supplied.')}</p></article>`).join('')||'<div class="q-empty-state"><strong>Provider registry unavailable.</strong><p>The terminal remains usable for local routing and outbound sources.</p></div>'}${sources.map(([id,source])=>`<article><div><strong>${esc(String(id).toUpperCase())}</strong><span class="q-status q-status--${esc(String(source?.truthState||source?.state||'unavailable').includes('live')?'live':'cached')}">${esc(String(source?.truthState||source?.state||'REFERENCE').toUpperCase())}</span></div><p>${esc(source?.attribution||source?.name||'External source')}</p></article>`).join('')}</div><details class="q-it-json"><summary>Inspect terminal evidence envelope</summary><pre>${esc(safeJson({releaseSha:network.releaseSha||null,providerCount:providers.length,sourceCount:sources.length,assistant:{mode:'local-route-guide',generative:false},externalAI:{apiAccess:false,embedAccess:false},community:{publicPosting:false,storage:'session-only'},execution:false}))}</pre></details></section>
  </section>`;
