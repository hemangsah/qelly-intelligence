...
The wider canonical destination vocabulary remains available through universal command search and the route inventory. Missing product families are recorded in `QELLY_FEATURE_MATRIX.csv`; they are not represented as fake placeholder routes.
...
The route registry assigns one shell kind and one product domain to every executable route. `QELLY_ROUTE_INVENTORY.csv` is the canonical audit output.
