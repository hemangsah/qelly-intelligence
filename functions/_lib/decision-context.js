  ...
  return {
    ...
    present:{
      ...
      calibration:{
        ...
        outcomeWindowOverlap:calibration.outcomeWindowOverlap===true,
...
function decisionTrace(graph,{multiTimeframe,tradeResearch,evidence,horizon,contradiction}){
  ...
  const satellite=[
    ...
    sourceNode('derivatives','derivatives','Funding and open-interest context',{
      ...
      methodology:'Current funding, open interest, mark/oracle basis, premium and 24h perpetual volume plus provider 24h price change are current-provider context; settled funding/premium history is same-provider context. LONG/SHORT build-up/covering/unwinding classification requires separately verified open-interest change.',
      limitations:['Historical open-interest change is not inferred when unavailable.','Funding, premium and basis never substitute for missing historical open-interest change.','LONG BUILD-UP / SHORT BUILD-UP / SHORT COVERING / LONG UNWINDING are descriptive price/OI quadrants, not ground-truth trader positioning.','Historical mark/oracle basis change is not inferred from premium history.','Liquidation flow and the price/OI quadrant remain unavailable without verified source history. The separate governed positioning state also remains unavailable until verified open-interest change exists.']
    }),
