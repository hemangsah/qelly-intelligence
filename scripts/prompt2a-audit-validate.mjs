...
await csv('QELLY_REQUESTED_FEATURE_STATUS.csv','feature_id','status',canonical,21);
...
if(!features.every(f=>master.some(m=>m.feature_id===f.feature_id)))failures.push('feature-summary-id-not-in-master');
if(providers.some(p=>!p.documentation_url||!p.terms_url||!p.recommended_action))failures.push('provider-registry-required-fields-missing');
...
const report={schemaVersion:1,generatedAt:new Date().toISOString(),head:process.env.QELLY_AUDIT_HEAD||'local',result:failures.length?'failed':'passed',metrics:{...metrics,masterFeatures:master.length,summaryFeatures:features.length,routes:routes.length,providers:providers.length,apis:apis.length},failures,warnings,files};await writeFile(path.join(out,'AUDIT_OUTPUT_VALIDATION.json'),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));if(failures.length)process.exit(1);
