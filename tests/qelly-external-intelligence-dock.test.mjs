...
test('external intelligence providers use official public display surfaces',()=>{
  ...
  assert.deepEqual(PROVIDER_PORTALS.map(item=>item.id),['forex-factory','coinglass','hypurrscan','arkham']);
...
test('Market Command exposes the lazy accessible intelligence dock',async()=>{
  ...
  for(const provider of ['Forex Factory','CoinGlass','HypurrScan','Arkham Intelligence'])assert.ok(PROVIDER_PORTALS.some(item=>item.name===provider));
