...
export function buildTradeResearch(graph,{requestedRr='auto',customRr=null,now=null}={}){
  ...
  const base={
    ...
    riskContext:{
      ...
      regime,
      eventRisk,
      liquidity:{state:liquidity?.state||'unavailable',currentOnly:liquidity?.currentOnly!==false,spreadBps:finite(liquidity?.spreadBps),spreadState:liquidity?.spreadState||'UNAVAILABLE',top1Imbalance:finite(liquidity?.top1Imbalance),top5Imbalance:finite(liquidity?.top5Imbalance),top10Imbalance:finite(liquidity?.top10Imbalance),imbalanceState:liquidity?.imbalanceState||'UNAVAILABLE',depthConsensus:liquidity?.depthConsensus||'UNAVAILABLE',micropriceBiasBps:finite(liquidity?.micropriceBiasBps),depthConcentrationTop1:finite(liquidity?.depthConcentrationTop1),depthConcentrationTop5:finite(liquidity?.depthConcentrationTop5),visibleBidCoverageBps:finite(liquidity?.visibleBidCoverageBps),visibleAskCoverageBps:finite(liquidity?.visibleAskCoverageBps),maxBidLevelGapBps:finite(liquidity?.maxBidLevelGapBps),maxAskLevelGapBps:finite(liquidity?.maxAskLevelGapBps),visibleDepthBands:Array.isArray(liquidity?.visibleDepthBands)?liquidity.visibleDepthBands.slice(0,3):[],liquidityVacuumState:liquidity?.liquidityVacuumState||'UNAVAILABLE_FROM_SINGLE_SNAPSHOT',reason:liquidity?.reason||null},
      derivatives:{
        ...
        prevDayPrice:finite(derivatives?.prevDayPrice),
        priceChange24hPct:finite(derivatives?.priceChange24hPct),
        openInterestChangePct:finite(derivatives?.openInterestChangePct),
        openInterestChangeState:derivatives?.openInterestChangeState||'UNAVAILABLE',
        positioningState:derivatives?.positioningState||'UNAVAILABLE',
        positioningAvailable:derivatives?.positioningAvailable===true,
