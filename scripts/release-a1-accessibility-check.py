...
try:
    ...
    routes=[('auth-login',False),('auth-register',False),('account-session',True),('onboarding',True),('discovery-hub',True),('live-markets',True),('identity-access',True),('security-evidence',True),('security-setup',True),('secure-import-vault',True)]
    ...
    log={'release':'24.0.0','generatedAt':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'method':'automated semantic, keyboard-entry and responsive regression; not an independent WCAG certification','routeCount':len(routes),'viewportCount':len(viewports),'checks':len(results),'passed':len(results)-len(failed),'failed':len(failed),'results':results,'status':'passed' if not failed else 'failed'}
    (ROOT/'validation'/'RELEASE_A1_ACCESSIBILITY_REGRESSION.json').write_text(json.dumps(log,indent=2)+'\n')
