...
const PREFERENCE_ROUTE_CONTRACT=Object.freeze({storage:'cloud-rls',persisted:true});
...
export async function onRequest(context){
  ...
  try{
    ...
    if(method==='GET'){
      return responseJson(request,env,{...uiPreferencesEnvelope(current),storage:PREFERENCE_ROUTE_CONTRACT.storage},200,{cookies:session.cookies,cache:'no-store'});
    }
    ...
    return responseJson(request,env,{...uiPreferencesEnvelope(row),storage:PREFERENCE_ROUTE_CONTRACT.storage,persisted:true},200,{cookies:session.cookies,cache:'no-store'});
  }catch(error){return errorResponse(request,env,error);}
}

export const __preferencesLayoutTest=Object.freeze({cleanPreferences:cleanUiPreferences,DEFAULTS:DEFAULT_UI_PREFERENCES,CONTRACT:PREFERENCE_ROUTE_CONTRACT});
