// V5.3 accepted terminal shell. Presentation/navigation only.
...
function systemText(){return 'UTC REFERENCE  |  Providers 5/6  |  Workspace: Institutional Research'}
function rail(){return `<aside class="q-v53-lock-shell-rail" aria-label="Qelly V5.3 product domains"><a class="q-v53-lock-shell-brand" href="#/feature-universe" aria-label="Qelly Intelligence"><span aria-hidden="true">Q</span><span>QELLY</span></a><nav class="q-v53-lock-shell-nav">${PRIMARY.map(([code,label,href])=>`<a href="${href}" class="${routeActive(code)?'is-active':''}" ${routeActive(code)?'aria-current="page"':''}><strong>${code}</strong><span>${label}</span></a>`).join('')}</nav><nav class="q-v53-lock-shell-nav q-v53-lock-shell-nav-secondary">${SECONDARY.map(([code,label,href])=>`<a href="${href}"><strong>${code}</strong><span>${label}</span></a>`).join('')}</nav></aside>`}
function shell(current){return `<div class="q-v53-lock-system"><span>${systemText()}</span><strong>READ ONLY · NO EXECUTION</strong></div><button type="button" class="q-v53-lock-command" data-v53-command aria-label="Search Qelly"><span aria-hidden="true">⌕</span><span>Search assets, routes, formulae, research, providers, audit…</span><kbd>CTRL K</kbd></button><div class="q-v53-lock-contextbar"><span>${current[0]} / ${current[1]}</span><span>SIMULATED REFERENCE DATA</span></div>${rail()}`}
