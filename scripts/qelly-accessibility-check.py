...
try:
    ...
    print('a11y server', base, flush=True)
    routes = [
        ('auth-login', False), ('auth-register', False), ('auth-recovery', False),
        ('account-session', True), ('onboarding', True), ('discovery-hub', True),
        ('live-markets', True), ('identity-access', True), ('security-evidence', True),
        ('security-setup', True), ('secure-import-vault', True), ('passkey-center', True),
        ('account-recovery', True), ('delivery-operations', True), ('platform-readiness', True),
        ('secret-rotation', True), ('quarantine-review', True), ('staging-assurance', True), ('decision-provenance', True)
    ]
    ...
    with sync_playwright() as p:
        ...
        for vname, viewport in viewports:
            ...
            payload = json.dumps({
                'email': f'a11y-{vname}-{stamp}@qelly.local',
                'password': 'Qelly-A11y-Foundation-2026!',
                'displayName': 'Accessibility Validator',
                'organizationName': f'Accessibility Org {vname} {stamp}',
                'workspaceName': 'Accessible Workspace',
                'locale': 'en-US', 'timezone': 'UTC', 'baseCurrency': 'USD'
            }).encode()
            ...
            for route, auth in routes:
                ...
                try:
                    ...
                    failures = ['render-failure']; errors.append(str(exc))
    ...
    log = {
        'productVersion': '0.9.0-preview.1', 'generatedAt': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
        'method': 'automated semantic, keyboard-entry and responsive regression; not an independent WCAG certification',
        'routeCount': len(routes), 'viewportCount': len(viewports), 'checks': len(results),
        'passed': len(results) - len(failed), 'failed': len(failed), 'results': results,
        'status': 'passed' if not failed else 'failed'
    }
    (ROOT / 'validation' / 'QELLY_ACCESSIBILITY_REGRESSION.json').write_text(json.dumps(log, indent=2) + '\n')
    print(json.dumps({'productVersion': log['productVersion'], 'checks': log['checks'], 'passed': log['passed'], 'failed': log['failed'], 'failures': failed}, indent=2))
    if failed:
        raise SystemExit(1)
finally:
    proc.terminate()
