...
const studioFiles=[
  path.join(studioDir,'theme-studio-desktop-dark.png'),
  path.join(studioDir,'theme-studio-desktop-light.png'),
  path.join(studioDir,'theme-studio-preview-stage.png')
];
...
const beforePinkFogRatio=missing.includes('01-light-mode/before-rejected-pink-fog.png')?null:await pinkFogRatio(beforeFile);
const correctedPinkFogRatio=missing.includes('01-light-mode/corrected-porcelain-daylight-full.png')?null:await pinkFogRatio(correctedFile);
const truthfulBeforeAfter=Number.isFinite(beforePinkFogRatio)&&Number.isFinite(correctedPinkFogRatio)&&beforePinkFogRatio>correctedPinkFogRatio;
const evidencePassed=missing.length===0&&darkThemePngs.length===13&&lightThemePngs.length===13&&commandPassed&&truthfulBeforeAfter;

await json(path.join(reportsDir,'EVIDENCE_COMPLETENESS_QA.json'),{
  result:evidencePassed?'passed':'failed',
  baselineCommit,reviewCommit,
  themeScreenshots:{dark:darkThemePngs.length,light:lightThemePngs.length},
  lightMode:{beforePinkFogRatio,correctedPinkFogRatio,before:'01-light-mode/before-rejected-pink-fog.png',corrected:'01-light-mode/corrected-porcelain-daylight-full.png',comparison:'01-light-mode/before-vs-corrected-porcelain-daylight.png'},
  desktopStudio:studioFiles.map((file)=>path.relative(artifact,file).split(path.sep).join('/')),
  selectedCommand:'08-command-palette/selected-unique-result.png',
  commandPalette:{result:commandQa?.result,duplicateLabels:commandDuplicates},
  missing
});
await writeFile(path.join(reportsDir,'EVIDENCE_COMPLETENESS_QA.md'),`# Evidence Completeness QA\n\n- Result: **${evidencePassed?'passed':'failed'}**.\n- Exact rejected baseline: \`${baselineCommit}\`.\n- Theme-family screenshots: ${darkThemePngs.length} dark + ${lightThemePngs.length} light.\n- Pink-fog ratio: ${beforePinkFogRatio} before → ${correctedPinkFogRatio} corrected.\n- Desktop Theme Studio: dark, light and preview-stage captures included.\n- Command palette: selected unique-result capture included; duplicate labels: ${commandDuplicates.length}.\n- Missing evidence files: ${missing.length}.\n`,'utf8');

const validationPath=path.join(reportsDir,'VALIDATION_SUMMARY.json');
...
validation.gates={...validation.gates,evidenceCompleteness:evidencePassed,truthfulBeforeAfter,desktopStudioEvidence:studioFiles.every((file)=>!missing.includes(path.relative(artifact,file).split(path.sep).join('/'))),selectedCommandEvidence:commandPassed&&!missing.includes('08-command-palette/selected-unique-result.png')};
validation.metrics={...validation.metrics,beforePinkFogRatio,correctedPinkFogRatio};
validation.result=Object.values(validation.gates).every(Boolean)?'passed':'failed';
await json(validationPath,validation);

await writeFile(path.join(reportsDir,'LIGHT_MODE_QA.md'),`# Light Mode QA\n\n- Result: **${validation.gates.lightModeNoPinkFog&&truthfulBeforeAfter?'passed':'failed'}**.\n- Exact rejected baseline commit: \`${baselineCommit}\`.\n- Measured pink-fog ratio: ${beforePinkFogRatio} before → ${correctedPinkFogRatio} corrected.\n- Before, corrected and side-by-side evidence are included under \`01-light-mode/\`.\n- Porcelain canvas, chart and table remain high-contrast and semantically neutral.\n`,'utf8');
...
const manifestFile=path.join(reportsDir,'ARTIFACT_MANIFEST.json');
const preManifest=await listFiles(artifact);const checksumLines=[];
for(const file of preManifest){if(file.absolute===checksumFile||file.absolute===manifestFile)continue;const bytes=await readFile(file.absolute);checksumLines.push(`${createHash('sha256').update(bytes).digest('hex')}  ${file.relative}`);}
checksumLines.sort();await writeFile(checksumFile,`${checksumLines.join('\n')}\n`,'utf8');
const finalFiles=await listFiles(artifact);const manifestItems=[];
for(const file of finalFiles){if(file.absolute===manifestFile)continue;const bytes=await readFile(file.absolute);manifestItems.push({path:file.relative,bytes:bytes.length,sha256:createHash('sha256').update(bytes).digest('hex')});}
await json(manifestFile,{schemaVersion:2,artifact:'qelly-theme-intelligence-visual-correction-review',commit:reviewCommit,baselineCommit,generatedAt:new Date().toISOString(),result:validation.result,files:manifestItems});
assert(validation.result==='passed',`Evidence finalization failed: ${JSON.stringify({missing,darkThemes:darkThemePngs.length,lightThemes:lightThemePngs.length,commandPassed,beforePinkFogRatio,correctedPinkFogRatio,validation},null,2)}`);
