...
async function renderLegacyDecisionProvenance(main,deps){
  ...
  const draw=()=>{
    ...
    form?.querySelectorAll('input[type="range"]').forEach((input)=>{input.addEventListener('input',()=>input.closest('label')?.querySelector('output')?.replaceChildren(`${input.value}%`));input.addEventListener('change',()=>document.dispatchEvent(new CustomEvent('qelly:product-event',{detail:{name:'decision_range_selected',properties:{route:'decision-provenance',feature:input.name==='scenarioMove'?'scenario_move':'confidence',action:'adjust'}}})));});
    form?.addEventListener('submit',(event)=>{event.preventDefault();const data=new FormData(form);result=evaluateDecision({assetId:data.get('assetId'),horizon:data.get('horizon'),risk:data.get('risk'),evidenceConfidence:Number(data.get('evidenceConfidence')),scenarioMove:Number(data.get('scenarioMove')),thesis:data.get('thesis'),invalidationCondition:data.get('invalidationCondition')});if(graphSource==='local'){graph=buildLocalDecisionGraph(result);selectedNodeId=defaultGraphNodeId(graph);}document.dispatchEvent(new CustomEvent('qelly:product-event',{detail:{name:'decision_explain',properties:{route:'decision-provenance',feature:'decision_intelligence',action:'run'}}}));draw();toast('Decision-support record recalculated',{tone:'success'});});
