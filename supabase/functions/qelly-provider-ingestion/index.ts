...
type RateDay={date:string;observedAt:string;rates:Record<string,number>};
...
function parseDays(xml:string,minDays=1){
  const days:RateDay[]=[];
  ...
  if(days.length<minDays)throw new Error(`ECB source failed schema threshold: ${days.length}/${minDays} days`);
  return days;
}
...
Deno.serve(async(req)=>{
  ...
  if(hadHistoryBackfill){
    history=await trySource(ECB_DAILY_URL,7000,1);sourceUrl=history?ECB_DAILY_URL:"";
    if(!history){history=await trySource(ECB_HISTORY_URL,14000,10);sourceUrl=history?ECB_HISTORY_URL:"";}
  }else{
    history=await trySource(ECB_HISTORY_URL,14000,10);sourceUrl=history?ECB_HISTORY_URL:"";
    if(!history){history=await trySource(ECB_DAILY_URL,7000,1);sourceUrl=history?ECB_DAILY_URL:"";}
  }

  if(!history){
    await admin.from("qelly_data_quality_events").insert({provider_id:provider.id,event_type:"error",severity:"material",truth_state:"error",details:{stage:"provider-ingestion",actor:"scheduler",attempts,cache:cached?"present":"missing"}});
    const staleUntil=cached?.stale_until?new Date(cached.stale_until).getTime():0;
    const cachedCount=Object.keys(cached?.payload?.rates||{}).length;
    if(staleUntil>now.getTime()&&cachedCount>=10){
      return reply(200,{ok:true,provider:PROVIDER_KEY,reused:true,stale:true,truthState:"delayed_provider",observationTime:cached.observation_time,ingestionTime:cached.ingestion_time,count:cachedCount,historyDays:Number(cached.payload?.historyDays)||0,historyPointCount:Number(cached.payload?.historyPointCount)||0,attempts,actor:"scheduler"});
    }
    return reply(502,{ok:false,error:"UPSTREAM_PROVIDER_FAILURE",provider:PROVIDER_KEY,attempts});
  }
  ...
  for(const quote of currencies){
    ...
    const body={provider_id:provider.id,instrument_id:instrument.id,provider_symbol:providerSymbol,provider_venue:"ECB reference rates",provider_identifier:{base:"EUR",quote},mapping_status:"verified",mapping_evidence:{source:sourceUrl,observation_date:latest.date,history_days:history.length},verified_at:ingestedAt,updated_at:ingestedAt};
  ...
  const historyBackfilled=hadHistoryBackfill||history.length>=10;
  ...
  const {error:cacheError}=await admin.from("qelly_provider_cache").upsert({provider_id:PROVIDER_KEY,cache_key:CACHE_KEY,payload:{base:"EUR",date:latest.date,rates:latest.rates,source:sourceUrl,history90dBackfilled:historyBackfilled,historyDays:historyBackfilled?Math.max(Number(cached?.payload?.historyDays)||0,history.length):history.length,historyPointCount:historyBackfilled?Math.max(Number(cached?.payload?.historyPointCount)||0,points.length):points.length},truth_state:"delayed_provider",observation_time:latest.observedAt,ingestion_time:ingestedAt,expires_at:expiresAt,stale_until:staleUntil,attribution:provider.attribution,license:"provider-registry:commercial_allowed+redistribution_allowed",updated_at:ingestedAt},{onConflict:"provider_id,cache_key"});
  if(cacheError)return reply(503,{ok:false,error:"PROVIDER_CACHE_PERSIST_FAILED"});

  return reply(200,{ok:true,provider:PROVIDER_KEY,reused:false,stale:false,truthState:"DELAYED",observationTime:latest.observedAt,ingestionTime:ingestedAt,count:currencies.length,historyDays:history.length,historyPointCount:points.length,historyBackfilled,source:sourceUrl===ECB_DAILY_URL?"ECB euro foreign exchange reference rates · daily":"ECB euro foreign exchange reference rates · 90-day history",attempts,actor:"scheduler"});
});
