...
test('global chat drawer exposes grounded context and reliability controls',async()=>{
  const [assistant,css,endpoint,decisionBridge]=await Promise.all([read('apps/web/public/assets/ai/qelly-chat.mjs'),read('apps/web/public/assets/ai/qelly-chat.css'),read('functions/api/v1/intelligence/chat.js'),read('apps/web/public/assets/decision-context-bridge.mjs')]);
  for(const phrase of ['data-q-ai-asset','data-q-ai-timeframe','data-q-ai-calculator','data-q-ai-copy-sources','data-q-ai-stop','data-q-ai-retry','QELLY tool receipts','unvalidated streaming disabled','freshness'])assert.match(assistant,new RegExp(phrase));
  assert.match(assistant,/storeDecisionContext/);
  assert.match(decisionBridge,/qelly\.decision\.chat-context\.v1/);
