...
const runtime=fs.readFileSync('apps/web/public/assets/qelly-ui-lock-v5-3.mjs','utf8');
...
test('Time Series mobile containment binds to the actual V5.3 runtime attribute',()=>{
  assert.match(runtime,/root\.dataset\.uiLockV53='active'/);
  assert.match(css,/@media\(max-width:720px\)[\s\S]*?html\[data-ui-lock-v53="active"\] #series-grid \.q-grid-scroll\{/);
  ...
  assert.doesNotMatch(css,/html\[data-ui-lock-v53="active"\] \.q-grid-scroll\{[\s\S]{0,160}max-height:clamp\(320px,56dvh,520px\)/);
});

test('Time Series repair does not activate the broader legacy V5.3 selector contract',()=>{
  assert.doesNotMatch(css,/html\[data-ui-lock-v53="active"\] body\{/);
  assert.doesNotMatch(css,/html\[data-ui-lock-v53="active"\] \.q-global-strip\{/);
});
