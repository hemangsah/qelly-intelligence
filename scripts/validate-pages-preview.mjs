...
const requiredFiles=[
  'index.html','404.html','qelly-config.js','BUILD_INFO.json','assets/app.js','assets/static-preview-api.mjs','assets/persona-profiles.mjs','assets/shell-foundations.mjs','assets/icon-registry.mjs','assets/qelly-foundations.css','assets/ui-rescue.css','assets/qelly-premium-reset.css','assets/premium-foundations.css','assets/premium-rankings.css','assets/premium-table.css','assets/premium-chart.css','assets/premium-mobile.css','assets/routes/asset-rankings.mjs','assets/routes/asset-rankings-premium.mjs','assets/routes/asset-rankings-data.mjs','assets/routes/asset-rankings-chart.mjs','assets/routes/asset-rankings-table.mjs','assets/tokens.css','assets/tokens.json','packages/accessibility/accessibility.mjs','packages/ui-primitives/primitives.mjs','packages/data-grid/data-grid.mjs','packages/charting/chart-shell.mjs'
];
...
async function walk(directory){
  ...
  for(const entry of await readdir(directory,{withFileTypes:true})){
    ...
    if(info.isDirectory())files.push(...await walk(absolute));else if(info.isFile())files.push(absolute);
  }
  return files;
}
...
for(const file of files){const text=await readFile(file,'utf8');for(const [rule,pattern] of forbiddenTextPatterns)assert(!pattern.test(text),`Potential ${rule} in Pages artifact: ${relative(file)}`);}
...
assert(index.includes('aria-label="Preview application state"'),'Static preview is missing the semantically labelled data-mode control');
assert(index.includes('qelly-premium-reset.css'),'Static preview is missing the premium reset stylesheet');
for(const match of index.matchAll(/(?:src|href)=["']\.\/([^"'?#]+)["']/g))assert(nameSet.has(match[1]),`index.html references a missing asset: ${match[1]}`);
...
const premiumRoute=await readFile(path.join(output,'assets/routes/asset-rankings-premium.mjs'),'utf8');
for(const truth of ['Static visual preview','Deterministic demo observations','backend unavailable','no production trading or persistence'])assert(premiumRoute.includes(truth),`Premium route is missing truth boundary: ${truth}`);
...
console.log(JSON.stringify({status:'pages-preview-validation-passed',label:'Static visual preview',output:path.relative(root,output),basePath:expectedBasePath,files:names.length,secretFindings:0,routes:['market','asset-rankings','asset/QI-CRYPTO-BTC','decision-provenance','feature-universe','about-qelly','theme-personas','auth-login']},null,2));
