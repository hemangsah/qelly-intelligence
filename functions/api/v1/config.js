import {CSRF_COOKIE,bootstrapContext,cookie,errorResponse,parseCookies,resolveSession,responseJson} from '../../_lib/runtime.js';
...
export async function onRequest(context){
  ...
  try{
    ...
    const qelly=session?await bootstrapContext(env,session):null;
    ...
    return responseJson(request,env,{
      ...
      defaultRoute:'market',
      csrf:{header:'X-Qelly-CSRF',token:qelly?csrf:null,mode:qelly?'double-submit-cookie':'unavailable-until-authenticated'},
      auth:{authenticated:Boolean(qelly),backendAvailable:true,productionIdentityEnabled:true,emailDeliveryAvailable:emailDelivery,registrationAvailable:emailDelivery,recoveryAvailable:emailDelivery,mode:'supabase-auth-cloudflare-facade'},
      ...
      liveTrading:false
    },200,{cookies:[...(session?.cookies||[]),...(qelly?[cookie(CSRF_COOKIE,csrf,{httpOnly:false,maxAge:60*60*8,sameSite:'Strict'})]:[])]});
