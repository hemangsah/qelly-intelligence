...
async function requireCsrf(runtime, request, sid, pathname) {
  ...
  if (request.headers['sec-fetch-site'] === 'cross-site') throw Object.assign(new Error('Cross-site state change blocked'),{status:403,code:'csrf_blocked'});
  ...
  return http.createServer(async (request,response)=>{
    ...
    try{
      ...
      if(request.method==='POST' && url.pathname==='/api/v1/auth/register') {
        ...
        const result=await runtime.productionAuthService.register(body,request,id);return json(response,201,{context:result.context,csrf:{header:'X-Qelly-CSRF',token:result.csrf},mode:'production-foundation'},id,{'Set-Cookie':result.cookie});
      }
      ...
      if(request.method==='GET' && url.pathname==='/api/v1/portfolio/overview') { await runtime.identityService.require(sid,'portfolio:read'); return json(response,200,runtime.portfolioService.overview(),id); }
      if(request.method==='GET' && url.pathname==='/api/v1/portfolio/holdings') { await runtime.identityService.require(sid,'portfolio:read'); return json(response,200,runtime.portfolioService.holdings(),id); }
      if(request.method==='GET' && url.pathname==='/api/v1/portfolio/performance') { await runtime.identityService.require(sid,'portfolio:read'); return json(response,200,runtime.portfolioService.performance({range:url.searchParams.get('range')??'1y'}),id); }
      if(request.method==='GET' && url.pathname==='/api/v1/portfolio/risk') { await runtime.identityService.require(sid,'portfolio:read'); return json(response,200,runtime.portfolioService.risk(),id); }
      ...
      if(request.method==='GET' && url.pathname==='/api/v1/portfolio/attribution') { await runtime.identityService.require(sid,'portfolio:read'); return json(response,200,runtime.portfolioService.attribution({range:url.searchParams.get('range')??'1y'}),id); }
      ...
      if(request.method==='GET' && url.pathname==='/api/v1/audit') { await runtime.identityService.require(sid,'audit:read'); return json(response,200,{items:await runtime.auditLedger.list(Number(url.searchParams.get('limit')??100))},id); }
...
export async function startServer({port=Number(process.env.PORT||4480),host=process.env.HOST||'127.0.0.1',runtimePath=defaultRuntimeDir,environment=process.env}={}){
...
if(process.argv[1] && fileURLToPath(import.meta.url)===path.resolve(process.argv[1])){
  const {host,port}=await startServer(); console.log(`Qelly Intelligence running at http://${host}:${port}`);
}
