...
import {MemorySessionStore,SupabasePublicBetaAdapter} from '../src/prompt2c/supabase-adapter.mjs';
...
test('adapter refuses unsafe endpoints and short keys',()=>{
  assert.throws(()=>new SupabasePublicBetaAdapter({url:'http://example.test',anonKey}));
  assert.throws(()=>new SupabasePublicBetaAdapter({url:'https://example.test',anonKey:'short'}));
});

test('sign-up uses an allowlisted redirect and never persists passwords',async()=>{
  let requestBody;
  const adapter=new SupabasePublicBetaAdapter({url:'https://project.supabase.co',anonKey,sessionStore:new MemorySessionStore(),redirectAllowlist:['https://qelly.example/auth/callback'],fetchImpl:mockFetch({'POST /auth/v1/signup':{body:{user:{id:'u1',email:'user@example.com'},session:null}}})});
...
test('sign-in persists only the provider session and authenticated RLS requests use bearer token',async()=>{
  ...
  const adapter=new SupabasePublicBetaAdapter({url:'https://project.supabase.co',anonKey,sessionStore:store,redirectAllowlist:['https://qelly.example'],fetchImpl});
...
test('account export aggregates only records visible through the authenticated RLS session',async()=>{
  ...
  const adapter=new SupabasePublicBetaAdapter({url:'https://project.supabase.co',anonKey,sessionStore:store,fetchImpl:async()=>new Response('[]',{status:200,headers:{'content-type':'application/json'}})});
...
test('expired session cannot be used for cloud reads',()=>{
  ...
  const adapter=new SupabasePublicBetaAdapter({url:'https://project.supabase.co',anonKey,sessionStore:store,fetchImpl:async()=>new Response('{}',{status:200})});
