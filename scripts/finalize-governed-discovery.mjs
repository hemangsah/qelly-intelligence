...
const migrations=Object.freeze([
  ...
  ["case 'converter': await renderConverter(main); break;","case 'converter': await import('./routes/governed-discovery.mjs').then(({renderGovernedConverter})=>renderGovernedConverter(main,{api,pageHead,stateBanner,escapeHtml,navigate,toast,state})); break;"],
  ["case 'trust-center': await renderTrustCenter(main); break;","case 'trust-center': await import('./routes/governed-discovery.mjs').then(({renderGovernedTrustCenter})=>renderGovernedTrustCenter(main,{api,pageHead,stateBanner,escapeHtml,navigate,toast,state})); break;"]
]);
...
export async function finalizeGovernedDiscovery({environment=process.env}={}){
  ...
  return {status:'governed-discovery-finalized',migrations:migrations.length};
}
