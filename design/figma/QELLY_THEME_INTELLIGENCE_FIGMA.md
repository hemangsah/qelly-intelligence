...
IBM Plex Sans Variable is the permanent target for every text and numeric role. Inter is only a local Figma fallback when IBM Plex is unavailable and must be resolved before approval. GT Eesti remains an inactive commercial licence gate.

The generator does not make network requests and does not claim to be an export from a hosted Figma file.
