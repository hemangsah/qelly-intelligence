...
export async function renderTimeSeriesV6(main,deps){
  ...
  const items=Array.isArray(summary.items)?summary.items.filter((item)=>String(item.assetClass||'').toLowerCase()==='fx'):[];
  ...
  const selected=items[0]?.symbol||'EURINR';
  main.innerHTML=`<section class="q-page q-v6-series-page">
    ...
    <section class="q-v6-series-summary">
      <div><span>Instruments</span><strong>${Number(summary.instruments)||0}</strong><small>Governed active master</small></div>
      <div><span>Series</span><strong>${Number(summary.series)||0}</strong><small>Normalized reference series</small></div>
  ...
  const load=async()=>{
    ...
    try{
      ...
      const points=Array.isArray(result.points)?result.points:[],last=points.at(-1),first=points[0],values=points.map((point)=>number(point.value)).filter((value)=>value!=null);
      ...
      main.querySelector('#v6-series-title').textContent=`${result.instrument?.symbol||symbol.value} · ${result.series?.metric||'reference rate'}`;
      main.querySelector('#v6-series-subtitle').textContent=`${result.instrument?.displayName||''} · ${result.series?.unit||''} · ${points.length} governed observations`;
      ...
      main.querySelector('#v6-series-evidence').innerHTML=`<dl class="q-v6-evidence-list"><dt>Truth state</dt><dd><span class="q-status q-status--${stateTone(truth)}">${escapeHtml(truth)}</span></dd><dt>Provider</dt><dd>${escapeHtml(result.provider?.displayName||result.provider?.providerKey||'Unknown')}</dd><dt>Observed</dt><dd>${escapeHtml(dateLabel(last?.observedAt))}</dd><dt>Ingested</dt><dd>${escapeHtml(dateLabel(last?.ingestedAt))}</dd><dt>Rights</dt><dd>${escapeHtml(`${result.provider?.commercialRightsStatus||'unknown'} / ${result.provider?.redistributionRightsStatus||'unknown'}`)}</dd><dt>Method</dt><dd>${escapeHtml(result.series?.methodology||'Not supplied')}</dd><dt>Attribution</dt><dd>${escapeHtml(result.provider?.attribution||'Not supplied')}</dd><dt>Execution</dt><dd>Disabled</dd></dl>`;
