...
const renderExternalResearch=(escapeHtml)=>`<section class="q-panel q-research-external-sources" aria-label="External research sources" data-external-research-boundary="display-and-outbound-only">
 <div class="q-panel-head"><div><span class="q-research-section-label">External research</span><h2>Official research surfaces</h2><p>Display/outbound research only. Opening a source does not ingest its values into Qelly analytics and does not create Qelly evidence automatically.</p></div><span class="q-status q-status--unavailable">external boundary</span></div>
 <div class="q-panel-body"><div class="q-research-evidence-grid">${EXTERNAL_RESEARCH.map((source)=>`<article class="q-research-evidence-card" data-external-source="${escapeHtml(source.id)}"><div class="q-research-evidence-card__head"><span class="q-status q-status--cached">${escapeHtml(source.mode)}</span><span>third-party / official source</span></div><h3>${escapeHtml(source.name)}</h3><p>${escapeHtml(source.purpose)}</p><div class="q-research-evidence-card__source"><span>Boundary</span><strong>${escapeHtml(source.note)}</strong></div><a class="q-button q-button--secondary" href="${escapeHtml(source.href)}" target="_blank" rel="noopener noreferrer" referrerpolicy="strict-origin-when-cross-origin">Open official source</a></article>`).join('')}</div></div>
</section>`;
...
export async function renderResearchWorkspace(main,deps){
 ...
 const referenceRows=references.length?references.map((item)=>`<div class="q-research-truth-row" data-provenance="research-reference"><span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(itemTypeLabel(item.type))}</small></span><code>${escapeHtml(item.referenceId)}</code></div>`).join(''):'<p class="q-research-empty">No external or canonical reference IDs are attached to this workspace yet.</p>';
 ...
 main.innerHTML=`<section class="q-page q-research-workspace-page" style="--q-v53-research-panel-2:color-mix(in srgb,var(--q-surface) 92%,var(--q-canvas))">${pageHead('Qelly Intelligence · Research & Evidence','Research Workspace','Persistent evidence boards combining assets, filings, research references, chart references, events and notes. External research remains isolated until the user intentionally captures a reference.',`<button class="q-button q-button--secondary" data-action="portfolio">Portfolio analytics</button><button class="q-button q-button--secondary" data-action="history">Version history</button><button class="q-button q-button--primary" data-action="create-workspace">New workspace</button>`)}${stateBanner()}
 ...
 ${renderExternalResearch(escapeHtml)}
