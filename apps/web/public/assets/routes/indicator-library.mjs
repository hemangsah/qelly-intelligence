...
export async function renderIndicatorLibrary(main,{pageHead,escapeHtml,navigate}){
  ...
  const card=(definition,{featured=false}={})=>`<a class="q-calculator-card q-indicator-card${featured?' is-featured':''}" href="#/indicator-detail/${encodeURIComponent(definition.indicatorId)}" data-indicator-id="${escapeHtml(definition.indicatorId)}">
    ...
    <div class="q-calculator-card__footer"><span>${escapeHtml(definition.requiredFields.join(', '))}</span><strong>View methodology <span aria-hidden="true">→</span></strong></div>
  </a>`;
  const featured=FEATURED_IDS.map((id)=>definitions.find((item)=>item.indicatorId===id)).filter(Boolean);

  main.innerHTML=`<section class="q-page q-calculator-center-page q-indicator-center-page">
    ...
    <section class="q-calculator-featured" aria-labelledby="indicator-featured-title">
      <div class="q-calculator-section-head"><div><p class="q-eyebrow">Widely used studies</p><h2 id="indicator-featured-title">Start with a familiar indicator</h2><p>Open a study to review its inputs, warm-up behavior, interpretation limits and deterministic sample evidence.</p></div></div>
      <div class="q-calculator-card-grid is-featured">${featured.map((definition)=>card(definition,{featured:true})).join('')}</div>
    </section>

    <section class="q-calculator-browser" aria-labelledby="indicator-browser-title">
      ...
      <div id="indicator-catalog" class="q-calculator-card-grid"></div>
