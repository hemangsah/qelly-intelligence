...
import { renderFilingWorkspace } from './routes/filing-workspace.mjs';
import { renderEventCalendar } from './routes/event-calendar.mjs';
import { renderComparisonLab } from './routes/comparison-lab.mjs';
import { renderWorkspaceWatchlist } from './routes/workspace-watchlist.mjs';
import { renderAlertCenter } from './routes/alert-center.mjs';
import { renderNotificationCenter } from './routes/notification-center.mjs';
import { renderScreenerLab } from './routes/screener-lab.mjs';
import { renderPortfolioAnalytics } from './routes/portfolio-analytics.mjs';
import { renderResearchWorkspace } from './routes/research-workspace.mjs';
import { renderOnboarding } from './routes/onboarding.mjs';
import { renderNotificationSchedules } from './routes/notification-schedules.mjs';
import { renderFormulaScreener } from './routes/formula-screener.mjs';
import { renderPortfolioAttribution } from './routes/portfolio-attribution.mjs';
import { renderImportCenter } from './routes/import-center.mjs';
...
import { renderAccountSession } from './routes/account-session.mjs';
import { renderSecuritySetup } from './routes/security-setup.mjs';
import { renderSecureImportVault } from './routes/secure-import-vault.mjs';
import { renderPasskeyCenter } from './routes/passkey-center.mjs';
import { renderAccountRecovery } from './routes/account-recovery.mjs';
import { renderDeliveryOperations } from './routes/delivery-operations.mjs';
import { renderPlatformReadiness } from './routes/platform-readiness.mjs';
import { renderSecretRotation } from './routes/secret-rotation.mjs';
...
async function renderMarket(main) {
  ...
  const rows=publicMarketRows(data.items);
  ...
  const kpis=data.kpis.map((item)=>`<article class="q-kpi"><div class="q-kpi-label">${escapeHtml(item.label)}</div><div class="q-kpi-value">${item.unit==='USD'?formatCompact(item.value):escapeHtml(String(item.value))}</div><div class="q-kpi-meta"><span>${escapeHtml(item.definition)}</span><span class="q-status q-status--${data.mode==='live-public'?'live':data.mode==='mixed'?'warning':'simulated'}">${escapeHtml(data.mode)}</span></div></article>`).join('');
  const heat=data.items.map((item)=>`<button class="q-heat-cell is-${(item.change24h??0)>=0?'positive':'negative'}" data-public-asset="${escapeAttribute(item.canonicalId)}"><strong>${escapeHtml(item.symbol)}</strong><span class="${(item.change24h??0)>=0?'is-positive':'is-negative'}">${(item.change24h??0)>=0?'+':''}${Number(item.change24h??0).toFixed(2)}%</span><small>${escapeHtml(item.source.qualityState)}</small></button>`).join('');
  main.innerHTML = `<section class="q-page">${pageHead('Public market launch','Market Overview','Read-only public exchange observations with canonical identifiers, explicit source evidence, freshness, cache state and deterministic degraded fallback.',actions)}${stateBanner()}<div class="q-truth-callout"><span class="q-status q-status--${data.mode==='live-public'?'live':data.mode==='mixed'?'warning':'simulated'}">${escapeHtml(data.mode)}</span><p>${escapeHtml(data.truthBoundary)}</p></div><div class="q-kpi-grid">${kpis}</div><div class="q-dashboard-grid"><div id="market-chart"></div><div class="q-dashboard-stack"><section class="q-panel"><div class="q-panel-head"><div><h2>Tracked-asset breadth</h2><p>Direction uses provider-reported 24-hour change</p></div><span class="q-status q-status--${data.mode==='live-public'?'live':'warning'}">${data.breadth.advancers} up · ${data.breadth.decliners} down</span></div><div class="q-panel-body"><div class="q-heatmap">${heat}</div></div></section><section class="q-panel"><div class="q-panel-head"><div><h2>Provider condition</h2><p>No fixture value is labelled live</p></div></div><div class="q-panel-body"><div class="q-context-block"><dl><dt>Provider</dt><dd>${escapeHtml(data.providerStatus.provider)}</dd><dt>Status</dt><dd>${escapeHtml(data.providerStatus.status)}</dd><dt>Last success</dt><dd>${escapeHtml(data.providerStatus.lastSuccessAt??'None in this process')}</dd><dt>Cache entries</dt><dd>${data.providerStatus.cacheEntries}</dd></dl></div></div></section></div></div><section class="q-panel"><div class="q-panel-head"><div><h2>Public asset table</h2><p>Sortable, keyboard-operable and source-labelled</p></div><span class="q-status q-status--${data.mode==='live-public'?'live':'warning'}">${data.items.length} assets</span></div><div id="market-grid"></div></section></section>`;
  ...
  new QellyChartShell(document.getElementById('market-chart'), { title:'BTC/USDT public price history', series, metadata:{source:candles.source.attribution,observedAt:candles.source.observedAt,receivedAt:candles.source.observedAt,confidence:candles.source.mode==='live-public'?.96:.72,freshnessClass:candles.source.mode==='live-public'?'live':'simulated'} });
  ...
  main.querySelector('[data-action="inspect-market"]').addEventListener('click', () => openContext({ title:'Public market evidence', source:data.providerStatus.provider, freshness:data.mode==='live-public'?'live':data.mode==='mixed'?'partial':'simulated', observedAt:data.generatedAt, receivedAt:data.generatedAt, confidence:data.mode==='live-public'?.96:.72, entitlement:'public-read', flags:[data.truthBoundary,'Read-only public endpoints','No trading or account endpoints'] }));
...
async function renderInstrumentMaster(main) {
  ...
  main.innerHTML=`<section class="q-page">${pageHead('Qelly Intelligence · Persistent canonical reference foundation','Instrument Master and Relationship Graph','Atomic local persistence now governs immutable QI-* identity, symbol history, venues, currencies, calendars, precision, identifiers and typed cross-asset relationships. It is not licensed production reference data.',`<button class="q-button q-button--secondary" data-action="resolve-qqqb">Resolve QQQB</button><button class="q-button q-button--primary" data-action="inspect-relationship">Inspect tokenized relationship</button>`)}${stateBanner()}
    ...
    <section class="q-panel"><div class="q-panel-head"><div><h2>Canonical registry</h2><p>Search by name, symbol, identifier, asset class or jurisdiction</p></div><label class="q-inline-search"><span class="sr-only">Filter instruments</span><input id="instrument-filter" type="search" placeholder="Filter registry"></label></div><div id="instrument-grid"></div></section>
