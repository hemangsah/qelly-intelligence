...
const required=['qelly-release.json','robots.txt','sitemap.xml','prompt2c-sw.js','support.html','legal/beta.html','legal/risk.html','legal/privacy.html','legal/terms.html','legal/legal.css','assets/prompt2c-public-beta.css','assets/prompt2c-public-beta.mjs'];
for(const file of required)assert((await stat(path.join(output,file))).isFile(),`Missing Prompt 2C public artifact: ${file}`);

const index=await read('index.html');
assert(index.includes('./assets/prompt2c-public-beta.css'),'Prompt 2C stylesheet not injected');
assert(index.includes('./assets/prompt2c-public-beta.mjs'),'Prompt 2C controller not injected');
const controller=await read('assets/prompt2c-public-beta.mjs');
...
assert(buildInfo.prompt2cPublicBeta===true,'Prompt 2C build flag missing');
assert(buildInfo.publicBetaMode==='QELLY GLOBAL PUBLIC BETA','Prompt 2C product mode missing');
assert(buildInfo.connectedCapabilitiesActivated===false,'Static artifact must not activate connected capabilities');

const robots=await read('robots.txt'),sitemap=await read('sitemap.xml'),serviceWorker=await read('prompt2c-sw.js');
...
console.log(JSON.stringify({status:'prompt2c-public-beta-artifact-passed',releaseSha:release.releaseSha,mode:release.mode,requiredFiles:required.length,connectedCapabilitiesActivated:false,publicUrl:release.fallbackUrl},null,2));
