...
test('audit ledger creates and verifies a hash chain',async()=>{const file=path.join(dir,'audit.ndjson');const ledger=new AuditLedger(file);await ledger.append({eventType:'one',actor:{type:'test',id:'1'},tenantId:'org-1',workspaceId:'ws-1'});await ledger.append({eventType:'two',actor:{type:'test',id:'1'},tenantId:'org-2',workspaceId:'ws-2'});const scoped=await ledger.list(100,{tenantId:'org-1',workspaceId:'ws-1'});assert.deepEqual(scoped.map((record)=>record.eventType),['one']);const result=await ledger.verify();assert.equal(result.valid,true);assert.equal(result.records,2);});
