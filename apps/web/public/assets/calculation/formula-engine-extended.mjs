...
export function listFormulaDefinitions({domain=null}={}){
  return [...listFoundationFormulaDefinitions(),...listFreshFormulaDefinitions()].filter(definition=>!domain||definition.domain===domain).map(definition=>({...definition}));
}
export function getFormulaDefinition(formulaId){return isFreshFormula(formulaId)?getFreshFormulaDefinition(formulaId):getFoundationFormulaDefinition(formulaId);}
