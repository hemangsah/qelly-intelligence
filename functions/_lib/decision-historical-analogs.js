...
export function buildDecisionHistoricalAnalogs(raw,{interval='15m',horizonBars=16,windowBars=100,limit=5}={}){
  ...
  return {
    ...
    method:'Nearest prior market-state windows using only pre-anchor trend, volatility, momentum and structure features. Candidates must clear a fixed 0.65 feature-similarity floor before any forward outcome is computed, and candidate stride is at least one full forecast horizon.',
    selectionPolicy:{
    ...
    leakageGuard:'No forward return, favorable excursion, adverse excursion, terminal price or time-to-resolution is used in feature distance, similarity acceptance or analog ranking.',
