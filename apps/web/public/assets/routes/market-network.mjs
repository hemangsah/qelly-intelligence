...
export async function renderGlobalMarketNetwork(main,deps){
  ...
  main.innerHTML=`<section class="q-page q-market-network" data-market-network="live-terminal-v8" data-network-experience="mission-control-v2" data-network-view="scan">
    ...
    <section class="q-mn-mission" aria-labelledby="q-mn-mission-title">
      <div class="q-mn-mission__copy"><p class="q-eyebrow">Cross-asset market orientation</p><h2 id="q-mn-mission-title">Decide whether a market move is broad—and whether every observation deserves trust.</h2><p>Global Market Network combines cross-asset orientation, independent source comparison and freshness evidence. It does not rank assets, build a thesis or execute a trade.</p><div class="q-mn-view-switch" role="toolbar" aria-label="Choose the market-network job">${Object.entries(VIEWS).map(([id,item],index)=>`<button type="button" data-network-view="${id}" aria-pressed="${index===0?'true':'false'}"><span>0${index+1}</span>${item.label}</button>`).join('')}</div></div>
    ...
    <section class="q-mn-workbench" data-network-section="scan">
      ...
      <aside class="q-mn-side">
        ...
        <section class="q-mn-panel"><div class="q-mn-panel-head"><div><h2>Data reliability</h2><p>What this page guarantees</p></div></div><div class="q-mn-panel-body"><div class="q-mn-live-board"><div class="q-mn-live-row"><div><strong>Source lineage</strong><small>Every displayed observation keeps its named source and freshness state</small></div><div class="q-mn-live-value"><b>ATTRIBUTED</b><span>TRACEABLE</span></div></div><div class="q-mn-live-row"><div><strong>Synthetic market values</strong><small>Never generated to fill missing price, candle, volume or market movement</small></div><div class="q-mn-live-value"><b>OFF</b><span>NO SUBSTITUTE</span></div></div><div class="q-mn-live-row"><div><strong>Trading execution</strong><small>This surface is for research and analytics only</small></div><div class="q-mn-live-value"><b>NOT AVAILABLE</b><span>READ ONLY</span></div></div><div class="q-mn-live-row"><div><strong>Unavailable sources</strong><small>Remain unavailable instead of being replaced with another source</small></div><div class="q-mn-live-value"><b>WITHHELD</b><span>TRUTHFUL</span></div></div></div><p class="q-mn-disclosure">Qelly displays only source-attributed observations permitted for this public research surface. Missing or unavailable sources stay unavailable. ECB observations are official reference rates, not executable market prices.</p></div></section>
      </aside>
    </section>
