...
test('calculation service limits batch size and returns structured unsafe-key validation',()=>{const service=new CalculationService();assert.throws(()=>service.batch({requests:Array(101).fill({formulaId:'cagr',inputs:{startValue:1,endValue:2,years:1}})}),/1–100/);const polluted=JSON.parse('{"formulaId":"fresh-present-value","inputs":{"__proto__":{"x":1},"futureValue":110,"rate":0.1,"periods":1}}');const result=service.calculate(polluted);assert.equal(result.status,'validation_error');assert.equal(result.validationErrors[0].code,'unsafe_key');assert.equal({}.x,undefined);});
