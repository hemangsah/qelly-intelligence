...
export class ProductionAuthService{
  constructor({repository,auditLedger,jobQueue=null,sessionSecret=process.env.QELLY_SESSION_SECRET,passwordPepper=process.env.QELLY_PASSWORD_PEPPER??'',secureCookies=process.env.NODE_ENV==='production',cookieSameSite=process.env.QELLY_SESSION_COOKIE_SAME_SITE??'Lax',sessionTtlMs=8*60*60*1000}={}){
    ...
    if(!['Lax','Strict','None'].includes(cookieSameSite))throw Object.assign(new Error('QELLY_SESSION_COOKIE_SAME_SITE must be Lax, Strict, or None'),{code:'session_cookie_same_site_invalid'});
    if(cookieSameSite==='None'&&!secureCookies)throw Object.assign(new Error('SameSite=None session cookies require Secure'),{code:'session_cookie_secure_required'});
    this.repository=repository;this.auditLedger=auditLedger;this.jobQueue=jobQueue;this.sessionSecret=sessionSecret;this.passwordPepper=passwordPepper;this.secureCookies=secureCookies;this.cookieSameSite=cookieSameSite;this.sessionTtlMs=sessionTtlMs;
  }
  ...
  async issueForPrincipal({userId,organizationId,workspaceId,authenticationMethod='password',request,rotatedFromSessionId=null}){
    ...
    return {sessionKey:this.sessionKey(token),session,csrf,cookie:serializeSessionCookie(signed,{secure:this.secureCookies,sameSite:this.cookieSameSite,maxAgeSeconds:this.sessionTtlMs/1000})};
  }
  async register(input,request,correlationId){
    ...
    const registrationDelivery=this.jobQueue?await this.jobQueue.enqueue({tenantId:registration.organizationId,workspaceId:registration.workspaceId,jobType:'notification.email',payload:{userId:registration.userId,tenantId:registration.organizationId,workspaceId:registration.workspaceId,destination:email,title:'Your Qelly workspace is ready',body:`Welcome to Qelly, ${displayName}. Your ${workspaceName} workspace has been created. If you did not register, contact Qelly support.`},idempotencyKey:`registration-welcome:${registration.userId}`}):null;
    return {context:await this.context(issued.sessionKey),csrf:issued.csrf,cookie:issued.cookie,registrationDelivery:registrationDelivery?{queued:true,jobId:registrationDelivery.job_id,dispatchPending:Boolean(registrationDelivery.dispatch_pending)}:null};
  }
