...
const tradeResearchMarkup=(data,escapeHtml)=>{
  ...
  const invalidations=trade.invalidations||{};
  const matrixMarkup=matrix.length?matrix.map(item=>'<article class="q-dpg-rr-card q-dpg-rr-card--'+escapeHtml(String(item.feasibility||'unavailable').toLowerCase().replace(/\s+/g,'-'))+'"><span>'+escapeHtml(item.label)+'</span><strong>'+money(item.target)+'</strong><small>'+escapeHtml(item.feasibility)+'</small><p>'+escapeHtml(item.feasibilityReason)+'</p><em>Net R:R: '+escapeHtml(item.costState==='UNAVAILABLE'?'unavailable — costs not fabricated':String(item.netRatio??'unavailable'))+(item.targetTouchProbability==null?' · Target-touch probability: uncalibrated':' · Target-touch probability: '+escapeHtml(String(item.targetTouchProbability)))+'</em></article>').join(''):'<p class="q-dpg-no-levels">No R:R matrix is available because the current evidence gate does not support a directional setup.</p>';
  const targetMarkup=targets.length?'<div class="q-dpg-target-ladder">'+targets.map(item=>'<span><em>T'+escapeHtml(String(item.rank))+' · '+escapeHtml(item.source||'MODEL')+'</em><strong>'+money(item.target)+'</strong><small>'+escapeHtml(item.label)+' · '+escapeHtml(item.feasibility)+'</small></span>').join('')+'</div>':'<p class="q-dpg-no-levels">No evidence-supported target ladder is available.</p>';
  const invalidationItems=[
    ['Price',invalidations.price],
    ['Structure',invalidations.structural],
    ['Evidence',invalidations.evidence],
    ['Time',invalidations.time],
    ['Event',invalidations.event],
    ['Regime',invalidations.regime]
  ].filter(([,item])=>item);
  const invalidationMarkup=invalidationItems.length?'<div class="q-dpg-invalidation-grid">'+invalidationItems.map(([label,item])=>'<article><span>'+escapeHtml(label)+'</span><strong>'+escapeHtml(String(item.state||'UNAVAILABLE').replaceAll('_',' '))+(Number.isFinite(Number(item.level))?' · '+money(item.level):'')+'</strong><small>'+escapeHtml(item.reason||item.invalidIf||'No additional condition supplied.')+'</small></article>').join('')+'</div>':'';
  ...
  return '<section class="q-dpg-trade-research"><header><div><small>FIND TRADE NOW · RESEARCH ONLY</small><h2>'+(trade.status==='VALID'?'Evidence-qualified setup':'No valid setup')+'</h2><p>'+escapeHtml(trade.reason)+'</p></div><span class="q-status q-status--'+status+'">'+escapeHtml(trade.status)+'</span></header>'+
    (trade.entry?'<div class="q-dpg-trade-summary"><article><span>Entry</span><strong>'+money(trade.entry.preferred)+'</strong><small>'+escapeHtml(trade.entry.method)+' · '+money(trade.entry.zone[0])+' – '+money(trade.entry.zone[1])+'</small><p>'+escapeHtml(trade.entry.trigger||'')+'</p></article><article><span>Stop / price invalidation</span><strong>'+money(trade.stop.price)+'</strong><small>'+escapeHtml(String(trade.stop.distancePct??'—'))+'% from price</small><p>'+escapeHtml(trade.stop.why||'')+'</p></article><article><span>Selected R:R</span><strong>'+(selected?escapeHtml(selected.label):'None')+'</strong><small>'+(selected?escapeHtml(selected.feasibility):'Not supported')+'</small><p>'+(selected?escapeHtml(selected.feasibilityReason||''):'No eligible target.')+'</p></article><article><span>Lifecycle</span><strong>'+escapeHtml(String(lifecycle.state||'NO_TRADE').replaceAll('_',' '))+'</strong><small>'+(trade.expiryAt?'Expires '+escapeHtml(displayTime(trade.expiryAt)):'Expiry unavailable')+'</small><p>'+escapeHtml(lifecycle.note||'Current observed state only.')+'</p></article></div>':'')+
    '<section class="q-dpg-trade-subsection"><header><div><small>TARGET FEASIBILITY</small><h3>Requested R:R matrix</h3></div><span>Presets are tests, not forced targets</span></header><div class="q-dpg-rr-grid">'+matrixMarkup+'</div></section>'+
    '<section class="q-dpg-trade-subsection"><header><div><small>STRUCTURALLY SUPPORTED TARGETS</small><h3>Target ladder</h3></div><span>Only supported levels shown</span></header>'+targetMarkup+'</section>'+
    (invalidationMarkup?'<section class="q-dpg-trade-subsection"><header><div><small>INVALIDATION MAP</small><h3>What cancels the setup</h3></div><span>Price · structure · evidence · time · event · regime</span></header>'+invalidationMarkup+'</section>':'')+
    '<div class="q-dpg-trade-boundary"><strong>Calibration boundary</strong><p>'+escapeHtml(trade.calibration)+'</p><p>'+escapeHtml(trade.costs?.reason||'Verified trading costs are unavailable, so net R:R is not fabricated.')+'</p></div>'+
