...
export async function initializeProductionFoundation(runtime,{runtimeDir,environment=process.env}={}){
  ...
  if(foundationEnabled){
    try{
      repository=await createProductionRepository({runtimeDir,mode:environment.QELLY_DATABASE_MODE??(environment.NODE_ENV==='production'?'postgres':'sqlite'),databaseUrl:environment.DATABASE_URL,sqlitePath:environment.QELLY_SQLITE_PATH,nodeEnv:environment.NODE_ENV,allowSqliteInProduction:truthy(environment.QELLY_ALLOW_SQLITE_IN_PRODUCTION,false)});
      ...
      authService=new ProductionAuthService({repository,auditLedger:runtime.auditLedger,sessionSecret:secret,passwordPepper:environment.QELLY_PASSWORD_PEPPER??'',secureCookies:environment.NODE_ENV==='production'});
      jobQueue=new ProductionJobQueue({repository,redisUrl:environment.REDIS_URL,mode:environment.QELLY_JOB_QUEUE_MODE??(environment.NODE_ENV==='production'?'redis':'database'),nodeEnv:environment.NODE_ENV,allowDatabaseQueueInProduction:truthy(environment.QELLY_ALLOW_DATABASE_QUEUE_IN_PRODUCTION,false)});
  ...
  if(repository?.constructor?.name==='PostgresProductionRepository') runtime.decisionProvenanceStore=new PostgresDecisionProvenanceStore({repository,auditLedger:runtime.auditLedger});
...
export async function productionFoundationHealth(runtime){
  ...
  const database=runtime.productionRepository?await runtime.productionRepository.health():{ok:false,driver:'unavailable',error:runtime.productionFoundation?.initializationError??'disabled'};
  const jobs=runtime.jobQueue?await runtime.jobQueue.health():{ok:false,driver:'unavailable',error:runtime.productionFoundation?.initializationError??'disabled'};
  const objectStorage=runtime.objectStorage?await runtime.objectStorage.health():{ok:false,driver:'unavailable',error:runtime.productionFoundation?.initializationError??'disabled'};
  ...
  const delivery={ok:Boolean(runtime.deliveryService)&&(!strict||(externalEmail&&externalWebhook)),driver:'delivery-adapters',providers,error:strict&&!(externalEmail&&externalWebhook)?'Production requires configured external email and signed-webhook adapters':null};
  const secrets=runtime.secretRotationService?await runtime.secretRotationService.status():{protector:{mode:'unavailable'},protectedRecordCount:0};
  const secretMode=String(secrets?.protector?.mode??'');
  const secretReady=!strict||secretMode.includes('configured');
  const scannerMode=String(objectStorage?.scanner??objectStorage?.scannerStatus?.mode??'');
  const scannerReady=!strict||scannerMode==='clamav-tcp-instream';
  const productionPolicy={strict,secretReady,scannerReady,externalEmail,externalWebhook,localFallbacksAllowed:!strict};
  const ready=Boolean(database.ok&&jobs.ok&&objectStorage.ok&&delivery.ok&&secretReady&&scannerReady);
  return {mode:runtime.productionFoundation,dependencies:{database,jobs,objectStorage,delivery,secrets},productionPolicy,ready};
}
