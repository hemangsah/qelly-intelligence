...
function hyperliquidTable(source,escapeHtml){
  ...
  if(!rows.length)return '<div class="q-mn-unavailable">Hyperliquid mids unavailable.</div>';
  ...
  main.innerHTML=`<section class="q-page q-market-network" data-market-network="live-terminal-v8">
    ...
    <section class="q-mn-workbench">
      <section class="q-mn-panel q-mn-chart-panel">
        ...
        <div class="q-mn-attribution">External display boundary. Qelly calculations, provider truth, alerts and decisions do not consume TradingView widget values.</div>
      </section>
      <aside class="q-mn-side">
        ...
        <section class="q-mn-panel"><div class="q-mn-panel-head"><div><h2>Production truth</h2><p>Connected runtime policy</p></div></div><div class="q-mn-panel-body"><p class="q-mn-disclosure">No market value is generated when an upstream provider fails or is rights-blocked. Coinbase and Binance stay unavailable until their intended commercial/display rights are verified. ECB observations are official reference rates, not tick execution prices.</p></div></section>
      </aside>
    </section>

    <section class="q-mn-panel"><div class="q-mn-panel-head"><div><h2>Professional research dock</h2><p>Official research/display destinations kept separate from Qelly analytical inputs.</p></div></div><div class="q-mn-panel-body q-mn-links">${researchLinks.map((item)=>`<a class="q-mn-link" href="${escapeHtml(item.url)}" target="_blank" rel="noopener noreferrer nofollow" title="${escapeHtml(item.note||item.mode)}"><strong>${escapeHtml(item.label)} ↗</strong><span>${escapeHtml(item.note||item.mode)}</span></a>`).join('')}</div></section>
