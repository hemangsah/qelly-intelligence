...
const PROVIDER_INTERVALS=Object.freeze({
  binance:Object.freeze(['1m','5m','15m','30m','1h','4h','1d']),
  coinbase:Object.freeze(['1m','5m','15m','1h','6h','1d'])
});
...
const supportedIntervals=(provider)=>PROVIDER_INTERVALS[String(provider)]||[];
const safeInterval=(provider,value)=>supportedIntervals(provider).includes(String(value))?String(value):(supportedIntervals(provider)[0]||'1m');
...
function unavailableResult({requestedProvider,symbol,interval,reason,termsState}={}){
  const normalizedProvider=String(requestedProvider||'');
  return {
    provider:normalizedProvider||null,
    requestedProvider:normalizedProvider||null,
    symbol:String(symbol||''),
    interval:safeInterval(normalizedProvider,interval),
    points:[],
    summary:summarize([]),
    source:{
      name:PROVIDER_UI[normalizedProvider]?.name||normalizedProvider||'No authorized provider',
      providerId:normalizedProvider||null,
      attribution:null,
      mode:'unavailable',
      observedAt:null,
...
export function liveMarketCatalog(){
  ...
  const providers=['binance','coinbase'].map(id=>{
    ...
    return {id,name:PROVIDER_UI[id].name,transport:[...PROVIDER_UI[id].transport],symbols:[...PROVIDER_UI[id].symbols],intervals:[...supportedIntervals(id)],enabled:Boolean(policy?.enabled),realtime:Boolean(policy?.enabled),realtimeAuthorized:Boolean(policy?.enabled),termsState:policy?.termsState||'unavailable',reason:policy?.reason||null,termsUrl:policy?.termsUrl||null};
  });
  const ecb=policies.get('ecb');
  return {
    ...
    governedReferenceData:{provider:'ecb',name:'European Central Bank reference rates',enabled:Boolean(ecb?.enabled),cadence:'daily-working-day-reference',termsState:ecb?.termsState||'unavailable',termsUrl:ecb?.termsUrl||null},
...
export async function liveMarketCandles(context,{provider='binance',symbol='BTCUSDT',interval='1m',limit=240}={}){
  ...
  const normalizedInterval=safeInterval(requestedProvider,interval);
  if(String(interval)!==normalizedInterval&&String(interval||'')!=='')return unavailableResult({requestedProvider,symbol,interval:normalizedInterval,reason:'provider_interval_not_supported',termsState:policy?.termsState||'unavailable'});
  if(!policy?.enabled)return unavailableResult({requestedProvider,symbol,interval:normalizedInterval,reason:policy?.reason||'provider_not_authorized_for_display',termsState:policy?.termsState||'unavailable'});
  const capability=await providerResult(context,requestedProvider,'candles',symbol,{interval:normalizedInterval,limit:clamp(limit,30,1000)});
  const normalized=normalizeProviderCandles(capability,requestedProvider,symbol,normalizedInterval);
  return normalized||unavailableResult({requestedProvider,symbol,interval:normalizedInterval,reason:capability?.fallbackReason||'provider_candles_unavailable',termsState:capability?.termsState||policy.termsState});
}
...
export async function liveMarketAsset(context,input={}){
  const result=await liveMarketCandles(context,input);
  return {...result,contract:'qelly-live-market-asset-v1'};
}

export function liveMarketStatus(){
  const catalog=liveMarketCatalog();
  return {enabled:catalog.liveModeEnabled,mode:catalog.liveModeEnabled?'rights-authorized-live':'unavailable-no-authorized-provider',providers:catalog.providers.map(({id,name,enabled,realtime,realtimeAuthorized,termsState,reason})=>({id,name,enabled,realtime,realtimeAuthorized,termsState,reason})),governedReferenceData:catalog.governedReferenceData,externalDisplay:catalog.externalDisplay,checkedAt:new Date().toISOString(),execution:false,fabricatedFallback:false};
}

export const __liveMarketTest=Object.freeze({summarize,safeInterval,supportedIntervals,unavailableResult});