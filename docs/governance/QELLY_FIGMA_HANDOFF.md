...
- 71 executable routes in `design/inventory/QELLY_ROUTE_INVENTORY.csv`;
- 431 governed frame records in `design/inventory/QELLY_SCREEN_MATRIX.csv`, through `QF-0431`;
...
The former handoff recorded 61 routes and 411 frames. The pre-fresh Prompt 2B repository had 66 routes and 421 frame records. The current route registry and deterministic generator define 71 routes and 431 frame records.
...
Re-running removes only pages marked by the generator itself. Repository validation uses `node --check design/figma/plugins/core/code.js`, `npm run inventory:design`, `npm run validate:design`, the route-frame mapping and representative browser screenshots. Native Figma execution remains a separate external action until an authenticated connector or desktop run is available.
