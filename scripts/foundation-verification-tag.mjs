...
assert(pages.status==='built',`Pages status is ${pages.status}`);
assert(String(pages.html_url||'').startsWith(BASE),'Pages URL mismatch');
await json('repository-state.json',{repository:repo.full_name,defaultBranch:repo.default_branch,main:main.commit.sha,pr11:{state:pr.state,merged:pr.merged,head:pr.head.sha,mergeCommit:pr.merge_commit_sha,mergedAt:pr.merged_at},pages,preservedBranches:[{name:rescue.name,sha:rescue.commit.sha},{name:recovery.name,sha:recovery.commit.sha}],mainChecks,approvedHeadChecks:headChecks});
...
const reducedContext=await browser.newContext({viewport:{width:390,height:844},reducedMotion:'reduce'});const reducedPage=await reducedContext.newPage();await reducedPage.goto(BASE,{waitUntil:'domcontentloaded',timeout:45000});await reducedPage.waitForTimeout(800);const reduced=await reducedPage.evaluate(()=>({reduced:matchMedia('(prefers-reduced-motion: reduce)').matches,motion:document.documentElement.dataset.motion,animations:getAnimations().filter((animation)=>animation.playState==='running').length}));assert(reduced.reduced,'reduced-motion environment not active');
